...
{
	public $table = 'SIMmJenisLayanan';
	protected $index_key = 'JenisLayanan';
	...
	public function dropdown_data($where = NULL)
	{
		if($items = $this->get_all(NULL, 0, $where))
		{
			...
			{
				$option_data[$item->{$this->index_key}] = $item->JenisLayanan;
			}
		}	
		return $option_data;
	}
	...
	public function to_list_html($first_label = '')
	{
		...
		if ($items = $query->result())
		{
			foreach($items as $item)
			{
				$option_html .= "<option value=\"{$item->{$this->index_key}}\">{$item->JenisLayanan}</option>";
			}
		}

...
{
	public $table = 'mUnitBisnis';
	public $index_key = 'UnitBisnisID';
	...
	public function dropdown_data($where = NULL)
	{
		if($items = $this->get_all(NULL, 0, $where))
		{
			...
			{
				$option_data[$item->{$this->index_key}] = $item->UnitBisnisName;
			}
		}	
		return $option_data;
	}
	
	public function to_list_data($first_label = '')
	{
		$items = $this->get_all();
		
		$populate = [0 => $first_label];
		if ($items = $this->get_all())
		{
			foreach($items as $item)
			{
				$populate[$item->UnitBisnisID] = $item->UnitBisnisName;
			}
		}
	...
	public function to_list_html($first_label = '')
	{
		...
		if ($items = $this->get_all())
		{
			foreach($items as $item)
			{
				$option_html .= "<option value=\"{$item->UnitBisnisID}\">{$item->UnitBisnisName}</option>";
			}
		}

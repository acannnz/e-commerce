	...
	<div class="col-md-12">
		<div class="panel panel-default">
            <div class="panel-heading">  
				<div class="row">
					<div class="col-md-6">
		                <h3 class="panel-title"><?php echo lang('heading:contract_list'); ?></h3>
					</div>
					<div class="col-md-6">
				...
				<div class="row">
					<div class="col-md-12">
						<div class="form-group">
							<table id="dt_contract" class="table table-bordered table-hover" width="100%" cellspacing="0">
								<thead>
									<tr>
										<th><?php echo lang('label:code') ?></th>
										<th><?php echo lang('label:name') ?></th>
										<th><?php echo lang('label:cooperation_type') ?></th>
										<th><?php echo lang('label:class') ?></th>
										<th><?php echo lang('label:date_end') ?></th>
										<th><?php echo lang('label:date_end') ?></th>
										<th><?php echo lang('label:status') ?></th>
								...
								<tfoot class="dtFilter">
									<tr>
										...
										<th><?php echo lang('label:cooperation_type') ?></th>
										<th><?php echo lang('label:class') ?></th>
										<th><?php echo lang('label:date_start') ?></th>
										<th><?php echo lang('label:date_end') ?></th>
										<th><?php echo lang('label:status') ?></th>
										<th style="width:65px;text-align:center;"><i class="fa fa-cog"></i></th>
									</tr>
...
(function( $ ){
						...
						var _datatable = _this.DataTable( {		
							...
							columns: [
									...
									},
									{
										...
										className: 'text-center',
										render: function(val){
											var status = val ? 'label-success' : 'label-primary';
											var text = val ? '<?php echo lang('global:active')?>' : '<?php echo lang('global:inactive')?>';
											
											return '<span class="label '+ status +'">'+ text +'</span>';
										}
									},
									{ 
										data: 'CustomerKerjasamaID',
										orderable: false,
										className: 'text-center',
										width: "120px",
										render: function ( val, type, row ){
												var buttons = "<div class=\"btn-group\" role=\"group\">";
													...
													<?php /*?>buttons += "<a href=\"javascript:;\" data-action-url=\"<?php echo base_url("{$nameroutes}/delete") ?>/" + val + "\" data-act=\"ajax-modal\" data-title=\"<?php echo lang('buttons:delete')?>\" title=\"<?php echo lang('buttons:delete') ?>\" class=\"btn btn-danger btn-xs\"> <i class=\"fa fa-trash\"></i></a>";<?php */?>
												buttons += "</div>";
		...
		$( document ).ready(function(e) {
				...
				_form.find("button[name=\"btn_search\"]").on("click", function(e){
						$( "#dt_contract" ).DataTable().ajax.reload();
				});					
					
				$("#dt_contract").DataTableInit();

...
{
	public $table = 'mSubKategori';
	protected $index_key = 'SubKategori_ID';
	...
	{	
		...
		{
			foreach($items as $item){ $populate[$item->SubKategori_ID] = $item->Nama_Sub_Kategori; }
		}
		
		return $populate;
	}
	
	public function to_list_html($parent_id = 0, $first_label = '')
	{
		$parent_id = (int) $parent_id;
		
		if ($parent_id == 0){ $items = $this->get_all(); }
		else { $items = $this->get_all(NULL,0,['Kategori_ID' => $parent_id]); }
		...
		if ($items)
		{
			...
			{
				$option_html .= "<option value=\"{$item->SubKategori_ID}\">{$item->Kode_Sub_Kategori} - {$item->Nama_Sub_Kategori}</option>";
			}
		}

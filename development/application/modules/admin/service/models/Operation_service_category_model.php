...
class Operation_service_category_model extends CI_Model
{
	public $table = 'SIMmKategoriOperasi';
	public $index_key = 'KategoriID';
	public $rules;
	...
	public function dropdown_data($where = NULL)
	{
		if($items = $this->get_all(NULL, 0, $where))
		{
			...
			{
				$option_data[$item->{$this->index_key}] = $item->KategoriName;
			}
		}	
		return $option_data;
	}
	...
	public function dropdown_html($where = NULL)
	{
		if($items = $this->get_all(NULL, 0, $where))
		{
			foreach($items as $item)
			{
				$option_html .= "<option value=\"{$item->{$this->index_key}}\">{$item->{$this->index_key}} - {$item->KategoriName}</option>";
			}
		}	
		return $option_html;
	}

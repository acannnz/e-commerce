...
class Account_model extends CI_Model
{
	...
	public function get_by(Array $where, $to_array = FALSE, $database = 'BO_1')
	{
		$this->db_bo = $this->load->database($database, TRUE);
		
		$this->db_bo->where($where);
		$query = $this->db_bo->get($this->table, 1);
		return (TRUE == $to_array) ? $query->row_array() : $query->row();
	}

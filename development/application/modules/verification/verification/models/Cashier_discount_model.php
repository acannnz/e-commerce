...
class Cashier_discount_model extends CI_Model
{
	public $table = 'SIMtrKasirDiscount';
	...
	public function get_collection($key, $to_array = FALSE)
	{
		...
		$_db_select = "
			...
			a.DokterID,
			a.KeteranganJasa, 
			a.DiscountWithException, 
			a.PersenHiden, 
			b.NamaDiscount,
			c.Nama_Supplier as Nama_Dokter,
			d.JasaID,
			d.JasaName,
			e.KelasID,
			e.NamaKelas
		...
		$query = $this->db
			->select( $_db_select )
			->from("{$this->table} a")
			->join("{$this->discount_model->table} b", "a.IDDiscount = b.IDDiscount", "INNER")
			->join("{$this->supplier_model->table} c", "a.DokterID = c.Kode_Supplier", "INNER")
			->join("{$this->service_model->table} d", "a.JasaID = d.JasaID", "INNER")
			->join("{$this->class_model->table} e", "a.KelasID = e.KelasID", "INNER")
	...
	public function for_dropdown( $all_section = FALSE )
	{
		...
		{
			$this->db->where('TipePelayanan','GUDANG');
		}

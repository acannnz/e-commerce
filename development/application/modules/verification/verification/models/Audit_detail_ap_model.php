...
class Audit_detail_ap_model extends CI_Model
{
	public $table = 'SIMtrAuditDetail_AP';  
	...
	public function get_collection($key, $to_array = FALSE)
	{
		...
		$query = $this->db
			->select('a.*, b.NamaKelas, c.Nama_Supplier')
			->from("{$this->table} a")
			->join("{$this->class_model->table} b", "a.KelasID = b.KelasID", "INNER")
			->join("{$this->supplier_model->table} c", "a.Supplier_ID = c.Supplier_ID", "INNER")

...
final class Posting_helper
{
	...
	public static function get_his_currency( $date, $_db_bo )
	{
		/*
			declare @server varchar(max);
			set @server = @@SERVERNAME;
			exec (@server).BO_1.dbo.CekHisCurrency'2019-02-02'
		*/
		$query = self::ci()->db->where("Tanggal", $date)->get( "{$_db_bo}TBJ_HisCurrency" );
				
		if( $query->num_rows() < 0 )
		{
			$server_name = self::ci()->db->query("SELECT @@SERVERNAME AS server")->row()->server;
			self::ci()->db->query("exec [{$server_name}].{$_db_bo}.CekHisCurrency '". $date ."' ");
		}
	...
	{
		$_ci = self::$_ci;
		self::get_his_currency( date('Y-m-d'), "{$_ci->BO_1->database}.dbo." );
		
		if(config_item('multi_bo') === 1)
		self::get_his_currency( date('Y-m-d'), "{$_ci->BO_2->database}.dbo." );
	...
	private static function _posting( $NoInvoice, $date )
	{
		...
		$IntHisCurrencyId = 0;
		$_his_currency = $_ci->{$_db_suffix}->select("MIN(HisCurrency_ID) AS ID ")
										->where('Tanggal <=', $date)
										->group_by(["HisCurrency_ID", "Tanggal"])
										->order_by('Tanggal', 'ASC')
										->get("{$_db_bo}TBJ_HisCurrency")
										->row();
		if( @$_his_currency->ID ):
		...
		if( $_get_audit ):
			...
			if( config_item('VerifikatorPostingKeAP') == 1 ):
				...
				$_db_select = "
					...
					FLOOR(Nilai_Hutang * (100 - Pajak) /100) AS Nilai_Hutang,
				...
				if( $_get_audit_ap ):
					foreach( $_get_audit_ap as $aap ): // aap = audit detail ap
						...
						$_insert_factur = [
							'Tgl_Faktur' => $aap->TglClosing, 
							'No_Faktur' => $aap->NoBuktiAP, 
							'Nilai_Faktur' => $aap->Nilai_Hutang, 
							...
							'Jenis_Pos' => 'FAP',
							'Nilai_Tukar' => 1,
							'HisCurrencyID' => $IntHisCurrencyId,
						...
						$CurNilaiHutang = $aap->Nilai_Hutang;

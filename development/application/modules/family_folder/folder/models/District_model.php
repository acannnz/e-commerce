...
{
	public $table = 'mKecamatan';
	protected $index_key = 'KecamatanID';
	...
	{
		$option_data = [];
		$db_where = ($parent_id !== 0) ? ['KabupatenID' => $parent_id] : NULL;
		
		if ($items = $this->get_all(NULL, 0, $db_where))
		{
			foreach($items as $item)
			{
				$option_data[ $item->{$this->index_key} ] = $item->KecamatanNama;
			}
		}

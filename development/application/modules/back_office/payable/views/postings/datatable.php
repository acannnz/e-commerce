...
<div class="row form-group">
	<div class="col-md-6">
    	...
    	<div class="form-group">
            <label class="col-lg-2 control-label"><?php echo lang('postings:supplier_label') ?></label>
            <div class="col-lg-3">
    ...
    <div class="col-md-6">
        ...
        <div class="form-group">
			<a href="<?php echo $posting_url ?>" data-toggle="form-ajax-modal" class="btn btn-block btn-danger"><b><i class="fa fa-random"></i> <?php echo lang("buttons:posting")?></b></a>
        </div>    
    </div>
</div>

<?php echo form_open( base_url("payable/postings/posting"), array("id" => "form_postings")) ?>
    ...
    <table id="dt-postings" class="table table-bordered table-striped table-sm" width="100%">
        <thead>
            <tr>
                ...
                <th><?php echo lang('postings:supplier_code_label') ?></th>
                <th><?php echo lang('postings:supplier_name_label') ?></th>
            </tr>
        </thead>        
        <tbody>
        </tbody>
		...
		$.fn.extend({
				DataTableGeneralLedgerPostings: function(){
						...
						var _datatable = _this.DataTable( {
							...
							responsive: true,
							ajax: {
									url: "<?php echo base_url("payable/postings/datatable_collection") ?>",
									type: "POST",
							...
							columns: [
									{ 
										...
										render: function ( val, type, row, meta ){
												return 	'<div class="checkbox">' +
															'<input type="checkbox"  id="row'+ meta.row +'" name="postings[row'+ meta.row +'][No_Bukti]" data class=\"post-check\" value ="'+ val +'" >' +
															'<label for="row'+ meta.row +'">&nbsp;</label>' +
															'<input type="hidden" class="source" name="postings[row'+ meta.row +'][source]" value="'+ row.source +'" disabled>' +
								...
								createdRow: function ( row, data, index ){
										$( "input.post-check", row ).on( "click", function(e){
												...
												$(this).prop("checked") 
													? $("input[name=\"postings["+ $(this).prop('id') +"][source]\"").prop("disabled", false)
													: $("input[name=\"postings["+ $(this).prop('id') +"][source]\"").prop("disabled", true);

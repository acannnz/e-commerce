...
class Memo extends Admin_Controller
{
	...
	protected $nameroutes = 'poly/outpatients/memo';
	...
	public function index( $NoReg, $SectionID )
	{
		$data = array(
				...
				'create_memo' => base_url("{$this->nameroutes}/item_create/{$NoReg}/{$SectionID}"),
				'delete_memo' => base_url("{$this->nameroutes}/item_delete"),
				'view_memo' => base_url("{$this->nameroutes}/item_view"),
			);

		$this->load->view( 'outpatient/form/memo', $data );		
	}
	...
	public function item_create($NoReg, $SectionID)
	{
		$item = (object) [
			'NoReg' => $NoReg, 
			'SectionID' => $SectionID,
			'SectionName' => $this->section_model->get_one($SectionID)->SectionName,
		];
		if( $this->input->post() ) 
		{
		...
		if( $this->input->is_ajax_request() )
		{
			...
			$this->load->view( 'outpatient/memo/form', $data );
		} 
	}
		...
		if( $this->input->post() ) 
		{
			...
			if( !$this->form_validation->run() )
			{
				$this->db->trans_begin();
					...
					$this->db->delete("SIMtrMemo", ["NoUrut" => $NoUrut] );				
	...
	public function item_view( $NoUrut )
	{
		...
		{		
			$data = [
				'item' => $this->poly_m->get_row_data("SIMtrMemo", ["NoUrut"  => $NoUrut]),
				'nameroutes' => $this->nameroutes,
				"is_edit" => TRUE,
			];
			
			$this->load->view( 'outpatient/memo/form_view', $data );
		} 
	}
}
...
class Prescriptions extends Admin_Controller
{
	...
	public function index($NoReg, $SectionID)
	{
		$data = array(
			...
			'edit_prescription' => base_url("{$this->nameroutes}/edit"),
	...
	public function edit($NoResep)
	{
		$item = $this->poly_m->get_prescriptions_data(["NoResep" => $NoResep]);
		$item[0]->NamaDokter = $this->supplier_model->get_by(['Kode_Supplier' => $item[0]->DokterID])->Nama_Supplier;

		// $item_detail = $this->poly_m->get_prescriptions_detail_data(["NoResep" => $NoResep]);
		// print_r($item);
		// exit;
		if (@$item[0]->SedangProses == 1) {
			$response = [
				"status" => 'error',
				"message" => 'Resep tidak bisa diedit, karena sedang proses realisasi Farmasi!!',
				"code" => 500
			];
			response_json($response);
		}
		...
		if ($this->input->post()) {
			$resep = $this->input->post('f');
			$resep['NoRegistrasi'] = $item[0]->NoRegistrasi;
			$resep['NoResep'] = $NoResep;
			$resep['User_ID'] = $this->user_auth->User_ID;
			...
			if ($this->form_validation->run()) {
				$this->db->trans_begin();
				$this->db->update("SIMtrResep", ["Jumlah" => $resep['Jumlah']], ["NoResep" => @$NoResep]);
				$this->db->delete("SIMtrResepDetail", ["NoResep" => $NoResep]);

				$detail = [];
				foreach ($details as $row) {
					$row['NoResep'] = $NoResep;
					$row['SectionID'] = $item[0]->SectionID;
		...
		if ($this->input->is_ajax_request()) {
			...
			// print_r($data);
			// exit;
			$this->load->view('outpatient/prescriptions/form_edit', $data);
		}
	}

...
{ 
	...
	public function index()
	{
		...
		{
			echo "<script language=\"javascript\">window.location=\"".base_url("{$this->nameroutes}/recap-stocks")."\";</script>";
			exit();
		} else
		{
			redirect();
	...
	public function dialog ( $type )
	{
		...
		$data = array(
				...
				"datepicker" => true,
				"form" => true,
				"export_url" => base_url("{$this->nameroutes}/recap-stocks/export/{$type}"),
				"lookup_products" => base_url("{$this->nameroutes}/recap-stocks/lookup_products")
			);
			
		if( $this->input->is_ajax_request())
		{
			$this->load->view(
				"polies/reports/recap_stocks/modal/dialog",
				array("form_child" => $this->load->view("polies/reports/recap_stocks/dialog", $data, true))
			);
		} else
		{
			$this->template
				...
				->set_breadcrumb( lang("reports:recap_stock_heading"), base_url("{$this->nameroutes}/recap-stocks") )
				->build('polies/reports/recap_stocks/dialog', (isset($data) ? $data : NULL));
		}
	}
	...
	private function export_pdf( $type )
	{
		...
		if ($this->input->post())
		{
			...
			$html_content =  $this->load->view( "polies/reports/recap_stocks/export/pdf", $data, TRUE ); 
	...
	public function lookup_products( $is_ajax_request=false )
	{
		...
		{
			$this->load->view( 'polies/reports/recap_stocks/lookup/products');
		} else
		{
			redirect( base_url( "{$this->nameroutes}/recap-stocks" ) );
		}
	}	

...
<style>
	.grand-total-label {
		text-align: right;
		/* Mengatur teks "Grand Total" ke kanan */
		padding-right: 10px;
		/* Jarak kanan dari teks ke kotak input */
		font-size: 15px;
	}

	.grand-total-input {
		width: 100%;
		/* Lebar kotak input agar mengisi seluruh kolom */
	}
</style>
...
<?php echo form_label("GRAND TOTAL" . ' *', 'input_total', ['class' => 'control-label col-lg-10 grand-total-label']) ?>
<div class="col-lg-2">
		...
		var _datatable_actions = {
			...
			get_component_service: function(params) {
				...
				if ($.isEmptyObject(service_component_temp[params.JasaID])) {
					...
					var totalBalance = parseFloat($("#grand_total").val().replace(/,/g, '')) || 0; // Ambil nilai totalBalance yang sudah ada, hapus koma jika ada
					var tarif = parseFloat(params.Tarif) || 0;

					totalBalance = totalBalance + tarif;
					$("#grand_total").val(totalBalance.toLocaleString('en-US', {
			...
			remove: function(params, fn, scope) {
				...
				var tarif = parseFloat(params.Tarif) || 0; // Ambil nilai tarif jasa yang dihapus
				var totalBalance = parseFloat($("#grand_total").val().replace(/,/g, '')) || 0; // Ambil nilai totalBalance yang sudah ada, hapus koma jika ada

				if (!isNaN(tarif)) {
					totalBalance = totalBalance - tarif; // Kurangkan tarif jasa yang dihapus dari totalBalance
					$("#grand_total").val(totalBalance.toLocaleString('en-US', {
						minimumFractionDigits: 2,
						maximumFractionDigits: 2
					}));

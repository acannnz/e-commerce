...
class Tank_auth
{
	 ...
	 * @return	array
	 */
	function create_user($username, $email, $password, $fullname, $company, $role, $phone, $email_activation)
	{
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username)) {
			...
			//$company_id = $this->_create_company($fullname,$email);
			...
			if (!is_null($res = $this->ci->users->create_user($data, $profile, !$email_activation))) {
	...
	function _create_company($company_name,$company_email){
		$data = array(

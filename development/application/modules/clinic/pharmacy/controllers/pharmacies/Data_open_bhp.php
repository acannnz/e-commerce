...
class Data_open_bhp extends Admin_Controller
{
	...
	public function index()
	{
		$data = [
			...
			'cek_bhp_url' =>  base_url("pharmacy/pharmacies/data_open_bhp/cek_bhp")
		];
		
		$this->load->view('pharmacies/datatable/open_bhp', (isset($data) ? $data : NULL));
	}

	
	//pengecekan bhp belum realisasi
	public function cek_bhp()
	{
		if ($this->input->is_ajax_request()) {
			...
			$queue_left = $this->db
				->select("COUNT(NoBuktiPOP) as queue_left")
				->where([
					"Realisasi_Farmasi" => 0,
					"Tanggal >=" => $params->date_from,
					"Tanggal <=" => $params->date_till
	...
	public function datatable_collection()
    {
		...
		$db_where['a.Realisasi_Farmasi'] = 0;		
		...
		// {
		// 	$db_where['a.Farmasi_SectionID'] = $this->input->post("SectionID");
		// }

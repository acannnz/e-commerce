...
class Pharmacy extends Admin_Controller
{
	...
	public function selling($NoResep = NULL, $Prosess = TRUE)
	{
		$item = [
			"KerjasamaID" => 3,
		...
		// print_r([$NoResep, $item['Proses']]);
		// exit;
		$this->db->update("SIMtrResep", array("SedangProses" => 1), array("NoResep" => $NoResep));
		...
		$data = [
			...
			"close_url" => base_url("pharmacy/close/") . $NoResep,
		];
	...
	public function close($NoResep = NULL)
	{

		$this->db->update("SIMtrResep", array("SedangProses" => 0), array("NoResep" => $NoResep));

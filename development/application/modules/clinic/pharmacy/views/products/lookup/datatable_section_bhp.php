		...
		$.fn.extend({
				DT_Lookup_CommonServices: function(){
						...
						var _datatable = _this.DataTable( {
							...
							responsive: true,
							ajax: {
									url: "<?php echo base_url("poly/products/datatable_product_section_bhp_collection") ?>",
									type: "POST",
									data: function( params ){
										if ($("#Farmasi_SectionID").size() > 0 ){
										params.Farmasi_SectionID = $("#Farmasi_SectionID").val();}
										if ($("#SectionID").size() > 0 ){
										params.SectionID = $("#SectionID").val();}
										if ($("#is_bhp").size() > 0 ){
										params.is_bhp = 1}
										params.JenisKerjasamaID = $("#JenisKerjasamaID").val() || 2;
										params.PasienKTP = $("#PasienKTP").val() || 1;

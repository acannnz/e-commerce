...
(function( $ ){
			...
			<?php if(config_item('use_websocket') == 'TRUE'): ?>
				socket.onmessage = function(e) {
					if (e.data == "refresh_queue") {
						...
						$.post('<?php echo $cek_bhp_url ?>',data_post,function(response, status, xhr) {
		...
		var search_datatable_o = {
			...
			check_open: function(count) {				
				...
				if(count > 0){
					$.notif_long(`Terdapat ${count} bhp yang belum direalisasi`);
				}
			}
		};
		...
		$.fn.extend({
				DataTable_DataWaitings: function(){
						...
						var _datatable = _this.DataTable( {
							...
							ajax: {
									url: "<?php echo base_url("pharmacy/pharmacies/data_open_bhp/datatable_collection") ?>",
									type: "POST",
									...
									{ 
										...
										render: function ( val, type, row ){
												var buttons = "<div class=\"btn-group pull-right\" role=\"group\">";
													buttons += "<a href=\"<?php echo base_url("pharmacy/selling_bhp") ?>/" + val + "\" title=\"Periksa Pasien\" class=\"btn btn-primary btn-xs\"><b><i class=\"fa fa-handshake-o\"></i> Realisasi</b></a>";
													buttons += "</div>";

...
final class inquiry_helper
{
	...
	public static function gen_opname_evidence_number(  )
	{
		...
		//$CI->load->model( "registrations/registration_m" );
		//$count = $CI->registration_m->count( array("DATE_FORMAT(registration_date, '%Y') =" => $date_y) );
		$count = @$CI->db->select("right(MAX(No_Bukti), 6) AS Max")
							->where(array(
									"DATEPART(YEAR, Tgl_Opname) =" => $NOW->format( "Y" ),
								))
							->get( "GD_trOpname" )
							->row()
							->Max
	...
	public static function get_request( $request_number )
	{
		$query = self::ci()->db->select("
								a.*, 
								b.SectionName NamaSectionAsal, 
								c.SectionName NamaSectionTujuan
							")
							->from("GD_tramprahan a")
							->join("SIMmSection b", "a.SectionAsal = b.SectionID", "LEFT OUTER")
							->join("SIMmSection c", "a.SectionTujuan = c.SectionID", "LEFT OUTER")
							->where("a.NoBukti", $request_number)
							->get();

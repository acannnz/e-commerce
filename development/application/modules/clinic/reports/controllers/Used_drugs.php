...
class Used_drugs extends ADMIN_Controller
{
	...
	protected $nameroutes = 'reports/used_drugs';
	...
	public function index()
	{
		...
		{
			echo "<script language=\"javascript\">window.location=\"".base_url("reports/used_drugs")."\";</script>";
			exit();
		} else
		{
			redirect( "reports/used_drugs/dialog" );
		}
	}
	...
	public function dialog ( $is_ajax = FALSE )
	{
		...
		{
			$this->load->view(
				"reports/reports/used_drugs/modal/dialog",
				array("form_child" => $this->load->view("reports/reports/used_drugs/dialog", $data, true))
			);
		} else
		{
			$this->template
				->set( "heading", $this->page )
				->set_breadcrumb("Laporan Penggunaan Obat", base_url("reports/used_drugs") )
				->set_breadcrumb( "Laporan Penggunaan Obat")
				->build('reports/reports/used_drugs/dialog', (isset($data) ? $data : NULL));
		}
	}
	...
	public function export_pdf()
	{
		if ($this->input->post())
		{
			...
			$html_content =  $this->load->view( "reports/reports/used_drugs/export/pdf", $data, TRUE ); 

...
EOSQL;
	...
	public static function get_examination($NoBukti)
	{
		...
		$db_select = <<<EOSQL
			...
			n.Nama_Supplier AS DirujukVendorName,			
EOSQL;
		...
		$query = $_ci->db->select($db_select)
					...
					->join("{$_ci->poly_m->table} o", "a.NoReg = o.RegNo", "LEFT OUTER")					
	...
	public static function create_examination($lab, $service, $service_component, $service_consumable, $checkout)
	{
		self::init();
		$_ci = self::$_ci;
		$_ci->load->model('laboratory_memo_model');
		...
		$_ci->db->trans_begin();
			...
			//update SIMtrMemoPenunjang
			$_ci->laboratory_memo_model->update_by(['SudahPeriksa' => 1], [ 'NoReg' => $lab['RegNo'], 'SectionTujuanID' => $lab['SectionID'] ]);
			
			if(!empty($lab['TransferSectionID'])):
				$_insert_registration_data = [
					'NoReg' => $lab['RegNo'],
					'Nomor' => 1 + (int) $_ci->laboratory_m->get_max_number( "SIMtrDataRegPasien", array("NoReg" => $lab['RegNo']), "Nomor" ),
					'Tanggal' => $lab['Tanggal'],
					'Jam' => $lab['Jam'],
	...
	public static function update_examination($lab, $service, $service_component, $service_consumable)
	{
		...
		$_ci->db->trans_begin();
			...
			$_ci->laboratory_memo_model->update_by(['SudahPeriksa' => 1], [ 'NoReg' => $lab['RegNo'], 'SectionTujuanID' => $lab['SectionID'] ]);
			if(!empty($lab['TransferSectionID'])):
				$lab_data = $_ci->laboratory_m->get_by(["RegNo" => $lab['RegNo'], "SectionID" => $lab['SectionID'], 'Batal' => 0]);
				...
				$_insert_registration_data = [
					'NoReg' => $lab['RegNo'],
					'Nomor' => 1 + (int) $_ci->laboratory_m->get_max_number( "SIMtrDataRegPasien", array("NoReg" => $lab['RegNo']), "Nomor" ),
					'Tanggal' => $lab_data->Tanggal,
					'Jam' => $lab_data->Jam,
					'SectionAsalID' => $lab['SectionID'],
					'SectionID' => $lab['TransferSectionID'],
					'KelasID' => $lab['KdKelas'],
					'DokterID' => $lab['TransferDokterID'],
					'NoAntri' => 1 + $_ci->laboratory_m->get_max_number( "SIMtrDataRegPasien", ["SectionID" => $lab['TransferSectionID'], 'DokterID' => $lab['TransferDokterID'], "Tanggal" => date("Y-m-d")], "NoAntri"),
					'SudahPeriksa' => 0,
					'RJ' => 1,
					'KelasAsalID' => $lab['KdKelas'],
				...
				if(empty($already_transfer)):
					...
					$_ci->registration_data_model->update_by($_insert_registration_data, ['SectionAsalID' => $lab['SectionID'], "SectionID" => $lab['TransferSectionID'], 'DokterID' => $lab['TransferDokterID']]);	
				endif;

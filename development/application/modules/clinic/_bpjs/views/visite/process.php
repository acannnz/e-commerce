...
<script type="text/javascript">
	...
	var state = {
		addVisite: {
			status: (visite.NoKunjungan !== null && visite.NoKunjungan !== '') ? true : false,
			data: {
				noKunjungan: visite.NoKunjungan
			}
		},
		saveVisite: {
			status: (visite.NoKunjungan !== null && visite.NoKunjungan !== '')
		},
		addService: {
			length: service.length,
			index: 0,
			progress: 0,
			increase: 100 / service.length * 2,
			status: false,
			data: {}
		},
		saveService: {
	...
	var bpjsVisiteProcess = {
		...
		addVisite: function() {
			...
			$.ajax({
				...
				if (response.status == false) {
					...
					$.alert_error('Tambah Kunjungan BPJS gagal :<br/>' + response.message);
					return false;
				}
				...
				$.alert_success('Tambah Kunjungan BPJS berhasil :<br/> ' + response.message);
				$("#visite").css({
					'width': '50%'
				});
		...
		addService: function() {
			...
			$("#service").css({
				'width': state.addService.progress + '%'
			});

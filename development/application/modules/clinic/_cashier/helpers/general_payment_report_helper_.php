...
final class general_payment_report_helper
{
	...
	public static function export_transaction_recap_by_section_doctor( $date_start, $date_end, $section_id, $doctor_id = NULL )
	{
		...
		$_sheet->mergeCells("J{$tb_row}:Q{$tb_row}");
		$sub_total_row = $tb_row - 1;
		// print_r($discount_col);exit;
		$_sheet->setCellValue("J{$tb_row}", "={$pendapatan_col}{$sub_total_row} - {$discount_col}{$sub_total_row}");
		$_sheet->getStyle("J{$tb_row}:Q{$tb_row}")->applyFromArray( self::_get_style( 'sum_value' ) );
		$_sheet->getStyle("J{$tb_row}:Q{$tb_row}")->applyFromArray( self::_get_style( 'currency' ) );
	...
	{		
		...
		foreach($query->result() as $row)
		{
			...
			// print_r($collection['data'][ $row->NoReg ]['Jasa'][$row->JasaName][$row->KomponenName]);exit;
		}

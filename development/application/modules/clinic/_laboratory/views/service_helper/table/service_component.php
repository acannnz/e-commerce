...
<div class="row form-group">
    <div class="col-md-12">
        <div class="table-responsive">
            <table id="dt_service_component" class="table table-bordered" width="100%">
                <thead>
                    <tr>
                        <th></th>
                        <th>Komponen</th>
...
(function( $ ){
		...
		var _datatable_actions = {
				...
				store_data: function(){
					...
					service = $("#dt_services").DataTable().row(indexRow).data();
					service.component_temp = data_component;
					$("#dt_services").DataTable().row(indexRow).data(service);
					
					console.log(service.component_temp);			
				},		
				...
				calculate_balance: function(){
					...
					_table.each(function(value, index){
						...
						total = parseFloat(total) + parseFloat(value.HargaBaru);
					});	
				...
				dt_service_component: function(){
						...
						_datatable = _this.DataTable( {
								...
								columns: [
										...
										{ 
											data: "HargaBaru", className: "text-right", 
											render: function( val, type, row ){
												return mask_number.currency_add(val);
											}
										},
										{ 
											data: "Disc", className: "text-center", 
		...
		$( document ).ready(function(e) {
				...
				if ( $.isEmptyObject(service_selected.component_temp) )
				{
					service_selected.component_temp = <?php print_r(json_encode(@$collection, JSON_NUMERIC_CHECK));?>;
				}
				console.log("Component: ", service_selected.component_temp);

            	$("#dt_service_component" ).dt_service_component();
				$("#dt_service_component").DataTable().clear().draw();
				$("#dt_service_component").DataTable().rows.add( service_selected.component_temp ).draw();

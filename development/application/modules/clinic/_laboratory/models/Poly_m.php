...
EOSQL;
	...
	public function get_service( $NoBukti, $item, $is_edit )
	{
		...
		if ( ! $is_edit )
		{
			...
			$query = $this->db->select('a.JasaID, a.JasaName')
							...
							->where(['b.SectionID' => config_item('section_id'), 'a.GroupJasaID' => 6])
							->limit(1)
							->get();
			$collection = [];				
			foreach( $query->result() as $row)
			{
				...
				$doctor = $this->db->select('b.Kode_Supplier, b.Nama_Supplier')							
								...
								->where(['a.NoReg' => $item->NoReg, 'a.SectionID' => config_item('section_id') ])
								->get()->row();
	...
	public function get_service_component( $params )
	{
		...
		if( in_array($params['JenisKerjasamaID'], [3, 4, 2, 9] ) ) // Sementara Tidak Untuk Kerjasama & BPJS tarifnya sama dengan Umum
		{
			...
			return
				$this->db->query("
							...
							from 
								dbo.GetDetailKomponenTarifNonKerjasama(". $params['ListHargaID'] .", '')
						")->result();
	...
	public function get_service_component_transaction( $NoBukti, $JasaID )
	{		
	...
	public function get_nurse( $where = NULL, $is_edit )
	{
		...
		if(!$is_edit)
		{
			$query = $this->db->select("Kode_Supplier, Nama_Supplier")
						->from("mSupplier")
						->where('Kode_Supplier', $this->session->userdata('nurse_id'))
						->get();

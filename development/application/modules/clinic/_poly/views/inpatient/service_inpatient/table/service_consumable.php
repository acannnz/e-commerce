...
<div class="row form-group">
	<a href="<?php echo @$lookup_service_consumable ?>" id="btn-service-consumable" data-toggle="lookup-ajax-modal" class="btn btn-primary btn-block"><b><i class="fa fa-plus"></i> Tambah BHP</b></a>
</div>
...
(function( $ ){
		...
		var service = $("#dt_services").DataTable().row(indexRow).data();
		var JasaID = service.JasaID;
		...
		var _datatable_actions = {
				...
				store_data: function(){
					...
					service.consumable_temp = data_consumable;
					$("#dt_services").DataTable().row(indexRow).data(service);
					
					console.log(service.consumable_temp);	
					},
		...
		$( document ).ready(function(e) {
				...
				if ( $.isEmptyObject(service.consumable_temp) )
				{
					...
					$.post("<?php echo $get_service_consumable ?>", data_post, function( response, status, xhr ){
						...
						if( response.collection ){
							...
							$("#dt_service_consumable").DataTable().rows.add( service.consumable_temp ).draw();				
						}
				...
				console.log("BHP: ", service.consumable_temp);

...
{ 
	...
	public function index( $type = 'outpatient' )
	{
		...
		$data = array(
				...
				'form' => TRUE,
				'datatables' => TRUE,
				'create_url' => base_url("poly/item-usage/create/{$type}")
			);
	...
	public function create( $type = 'outpatient' )
	{
	  	$location = $this->session->userdata($type);
		...
		if( $this->input->post() ) 
		{
			...
			if( !$this->form_validation->run() && $validation )
			{
				...
				$this->db->trans_begin();
					...
					$activities_description = sprintf( "%s # %s # %s # %s", "Input Pemakaian Barang.", $item->NoBukti, $section->SectionID, $this->user_auth->Nama_Asli );			
					...
					foreach( $detail as $val ):
						...
						$qty_last_stock = $this->item_usage_m->get_last_stock_warehouse_card( array("Lokasi_ID" => $section->Lokasi_ID, "Barang_ID" => $val["BarangID"]) );
		...
		if( $this->input->is_ajax_request() )
		{
			$data = array(
					...
					"section" => $section,
					"type" => $type,
					"lookup_product" => base_url("poly/item_usage/lookup_product"),
		...
		} else
		{
			$data = array(
					...
					"type" => $type,
					"section" => $section,
					"lookup_item" => base_url("poly/item_usage/lookup_item"),
					"create_url" => base_url("poly/item-usage/create/{$type}"),
					"form" => TRUE,
					"datatables" => TRUE,
				);
			...
			$this->template
				...
				->set_breadcrumb( lang("item_usage:breadcrumb"), base_url("poly/item-usage/{$type}") )
				->set_breadcrumb( lang("item_usage:create_heading") )
				->build('item_usage/form', $data);
		}
	}
	
	public function view( $type = 'outpatient', $NoBukti = 0 )
	{
		...
		if( $this->input->is_ajax_request() )
		{
			$data = array(
					...
					"cancel_link" => base_url("poly/item-usage/cancel/$item->NoBukti"),
					"is_ajax_request" => TRUE,
					"is_modal" => TRUE,
					"is_edit" => TRUE,
				);
		...
		} else
		{
			$data = array(
					...
					"type" => $type,
					"collection" => $collection,
					"cancel_url" => base_url("poly/item-usage/cancel/{$type}/$item->NoBukti"),
					"create_url" => base_url("poly/item-usage/create/{$type}"),
					"form" => TRUE,
					"datatables" => TRUE,
				);
			...
			$this->template
				...
				->set_breadcrumb( lang("item_usage:breadcrumb"), base_url("poly/item-usage") )
				->set_breadcrumb( lang("item_usage:view_heading") )
				->build('item_usage/view', $data);
		}
	}
	
	public function cancel( $type = 'outpatient', $NoBukti )
	{
		$location = $this->session->userdata($type);
		$section = $this->section_model->get_one($location['section_id']);
		$item = $this->item_usage_m->get_item_usage( $NoBukti );		
		...
		if( $this->input->post() ) 
		{
			...
			if( $item->NoBukti == $this->input->post( 'confirm' ) )
			{
				...
				$this->db->trans_begin();
					...
					// Insert User Aktivities
					$activities_description = sprintf( "%s # %s # %s # %s", "Cancel Pemakaian Barang.", $item->NoBukti, $section->SectionName, $this->user_auth->Nama_Asli );			
				...
				else
				{
					...
					redirect("poly/item-usage/view/{$type}/{$item->NoBukti}");
				}	
			}
	...
	public function datatable_collection( $state=false )
    {
		...
		{
			$db_where['a.SectionID'] = $this->input->post("section_id");
		}

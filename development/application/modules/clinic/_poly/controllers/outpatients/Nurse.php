...
class Nurse extends Admin_Controller
{
	...
	protected $nameroutes = 'poly/outpatients/nurse';
	...
	public function index( $NoBukti, $is_edit = FALSE )
	{
		$medics = $this->session->userdata('outpatient');
		$where = $is_edit ? ['NoBukti' => @$NoBukti]
						: ['Kode_Supplier' => @$medics['nurse_id']];
		
		$data = [
			'collection' => $this->poly_m->get_nurse( $where, $is_edit ),
			'nameroutes' => $this->nameroutes,
			'lookup_nurse' => base_url("{$this->nameroutes}/lookup_nurse")
		];

		$this->load->view( 'outpatient/form/nurses', $data );		
	}
	
	public function lookup_nurse( $is_ajax_request=false )
	{
		if( $this->input->is_ajax_request() || $is_ajax_request !== false )
		{
			$this->load->view( 'outpatient/lookup/nurses' );
		} 
	}

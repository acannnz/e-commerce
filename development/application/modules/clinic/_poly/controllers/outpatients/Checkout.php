...
class Checkout extends Admin_Controller
{
	...
	protected $nameroutes = 'poly/outpatients/checkout';
	...
	{
		$data = array(
				"collection" => $this->poly_m->get_checkout(["NoReg" => @$NoReg, "SectionAsalID" => $SectioniD, "TipePelayanan" => 'RJ']),
				'nameroutes' => $this->nameroutes,
				"section_dropdown" => base_url("{$this->nameroutes}/section_dropdown"),
				"doctor_dropdown" => base_url("{$this->nameroutes}/doctor_dropdown"),
				"time_dropdown" => base_url("{$this->nameroutes}/time_dropdown"),
				"get_queue" => base_url("{$this->nameroutes}/get_queue"),
			);

		$this->load->view( 'outpatient/form/checkout', $data );		
	}
	...
	public function lookup_checkout( $is_ajax_request=false )
	{
		...
		{
			$this->load->view( 'outpatient/lookup/checkout' );
		} 
	}
	...
	public function section_dropdown( $selected='', $section_asal)
	{
	
		if( $this->input->is_ajax_request() )
		{
			if( !empty( $section_asal ))
			{
				$this->db->where('SectionID !=', $section_asal);
			}
	...
	public function get_queue()
	{
		if ( $this->input->is_ajax_request() )		
		{
			...
			{
				$queue = (int) @max(array(++$queue1, ++$queue2));
			} else {
				$queue = 1;
			}

...
class Supporting_tool_model extends CI_Model
{
	public $table = 'SIMmAlatPenunjang';
	public $index_key = 'IDAlat';
	public $rules;
	...
	public function to_list_data($db_where = NULL)
	{	
		if ($items = $this->get_all(NULL, 0, $db_where))
		{
			foreach($items as $item){ $populate[$item->SectionID] = $item->SectionName; }
		}
		
		return $populate;
	}
	
	public function to_list_html()
	{
		$option_html = "";		
		if ($items = $this->get_all())
		{
			...
			{
				$option_html .= "<option value=\"{$item->IDAlat}\">{$item->NamaAlat}</option>";
			}
		}

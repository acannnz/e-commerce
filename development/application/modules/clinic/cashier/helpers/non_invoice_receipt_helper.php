...
final class non_invoice_receipt_helper
{
	...
	public static function gen_evidence_number( )
	{
		...
		$query =  $CI->db->select("MAX(NoBukti) AS MAX")
							->where(array("Tipe" => "PNP","Tanggal >=" => $date_start,"Tanggal <=" => $date_end))
		...
		if (!empty($query->MAX))
		{
			...
			$number++;
			$number = (string) (sprintf(self::_gen_format_evidence_number(), $date_y, $date_m, $date_d, 'PNP', $number));	
		} else {
			$number = (string) (sprintf(self::_gen_format_evidence_number(), $date_y, $date_m, $date_d, 'PNP', 1));		
		}
	...
	public static function get_non_invoice_receipt( $NoBukti )
	{
		$CI = self::ci();	

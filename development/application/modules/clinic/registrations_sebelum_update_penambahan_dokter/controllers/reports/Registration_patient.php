...
class Registration_patient extends Admin_Controller
{ 
	protected $_translation = 'reports';	
	protected $nameroutes = 'registrations/reports/registration-patient';
	...
	public function dialog ( $ajax = FALSE)
	{
		...
		{
			$this->load->view(
				"reports/registration_patient/modal/dialog",
				array("form_child" => $this->load->view("reports/registration_patient/dialog", $data, true))
			);
		} else
		{
			$this->template
				...
				->build('reports/registration_patient/dialog', $data );
		}
	}

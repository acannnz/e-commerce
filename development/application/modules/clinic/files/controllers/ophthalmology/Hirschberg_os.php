...
class Hirschberg_os extends Public_Controller
{
	protected $_translation = "chart_file";	
	protected $_model = "chart_file_m";
	protected $_page = "files_ophthalmology_hirschberg_os";
	
	private $_file_type = "ophthalmology_hirschberg_os";
	
	public function __construct()
    {
        parent::__construct();
		
		$this->load->language( "ophthalmology/ophthalmology" );
	...
	public function update( $chart_number, $inline=FALSE )
	{
		...
		$data = array(
				"update_action" => base_url( "files/ophthalmology/hirschberg/os/update/{$chart_number}" ),
				"file_upload_action" => base_url( "files/ophthalmology/hirschberg/os/file_upload/{$chart_number}" ),
				"file_delete_action" => base_url( "files/ophthalmology/hirschberg/os/file_delete/{$chart_number}" ),
		...
		if( $this->input->is_ajax_request() )
		{
			...
			echo ($this->load->view( "ophthalmology/okuli_sinistra/form/update", $data, TRUE ));
			exit(0);
		}
		...
		if( TRUE == $inline )
		{
			return ($this->load->view( "ophthalmology/okuli_sinistra/form/update", $data, TRUE ));
		}
		...
		$this->template
			...
			->build('ophthalmology/okuli_sinistra/form/update', $data);
	}
	...
	public function file_upload( $chart_number )
	{
		...
		if( $this->input->is_ajax_request() )
		{
			header("Content-Type: text/plain; charset=utf-8");
			echo ($this->load->view( 
					'ophthalmology/okuli_sinistra/modal/file_upload', 
					array('form_child' => $this->load->view('ophthalmology/okuli_sinistra/form/upload', $data, TRUE)),
					TRUE
				));
			exit(0);
		}
		...
		$this->template
			...
			->build('ophthalmology/okuli_sinistra/form/upload', $data);
	}
	...
	public function file_delete( $chart_number )
	{
		...
		header("Content-Type: text/plain; charset=utf-8");
		echo $this->load->view( 'ophthalmology/okuli_sinistra/modal/file_delete', $data, TRUE );
		exit(0);
	}
	...
	protected function _file_upload( & $file )
    {
		...
		if( $_FILES )
		{
			...
			{
				$config[ 'upload_path' ]	= './resource/patients/ophthalmology/';
				$config[ 'allowed_types' ]	= 'jpg';
				$config[ 'file_name' ]		= sprintf('PATIENT_%s_HIRSCHBERG_OS', strtoupper(str_replace(".","-",$file->mr_number)));

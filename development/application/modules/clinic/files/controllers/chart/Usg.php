...
class Usg extends Public_Controller
{
	protected $_translation = "chart_file";	
	protected $_model = "chart_file_m";
	protected $_page = "files_chart_usg";
	
	private $_file_type = "ultrasonography";
	...
	public function update( $chart_number, $inline=FALSE )
	{
		...
		$data = array(
				"update_action" => base_url( "files/chart/usg/update/{$chart_number}" ),
				"file_upload_action" => base_url( "files/chart/usg/file_upload/{$chart_number}" ),
				"file_delete_action" => base_url( "files/chart/usg/file_delete/{$chart_number}" ),
		...
		if( $this->input->is_ajax_request() )
		{
			...
			echo ($this->load->view( "chart/usg/form/update", $data, TRUE ));
			exit(0);
		}
		...
		if( TRUE == $inline )
		{
			return ($this->load->view( "chart/usg/form/update", $data, TRUE ));
		}
		...
		$this->template
			...
			->build('chart/usg/form/update', $data);
	}
	...
	public function file_upload( $chart_number )
	{
		...
		if( $this->input->is_ajax_request() )
		{
			header("Content-Type: text/plain; charset=utf-8");
			echo ($this->load->view( 
					'chart/usg/modal/file_upload', 
					array('form_child' => $this->load->view('chart/usg/form/upload', $data, TRUE)),
					TRUE
				));
			exit(0);
		}
		...
		$this->template
			...
			->build('chart/usg/form/upload', $data);
	}
	...
	public function file_delete( $chart_number )
	{
		...
		header("Content-Type: text/plain; charset=utf-8");
		echo $this->load->view( 'chart/usg/modal/file_delete', $data, TRUE );
		exit(0);
	}
	...
	protected function _file_upload( & $file )
    {
		...
		if( $_FILES )
		{
			...
			{
				$config[ 'upload_path' ]	= './resource/patients/ultrasonography/';

...
class Regional_m extends CI_Model
{
	public $table = 'mRegional';
	public $index_key = 'Kode_Regional';
	public $rules;
	...
	public function dropdown_data($where = NULL)
	{
		if($items = $this->get_all(NULL, 0, $where))
		{
			...
			{
				$option_data[$item->Kode_Regional] = $item->Nama_Regional;
			}
		}	
		return $option_data;
	}
	...
	public function to_list_data($where = NULL, $first_label = '')
	{
		if ($where !== 0 && !empty($where)){ 
		...
		{
			foreach($items as $item){ $populate[$item->Kode_Regional] = $item->Nama_Regional; }
		}

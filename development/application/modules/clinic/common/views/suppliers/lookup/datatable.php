...
<div class="table-responsive">
    <table id="dt-lookup-suppliers" class="table table-sm table-bordered table-striped" width="100%">
        <thead>
            <tr>
                <th></th>
                <th>Kode </th>
...
(function( $ ){
		$.fn.extend({
				DT_Lookup_Suppliers: function(){
						...
						var _datatable = _this.DataTable( {
							...
							//scrollY: "200px",
							ajax: {
									url: "<?php echo base_url("common/suppliers/lookup_collection") ?>",
									type: "POST",
							...
							columns: [
									...
									{ data: "SubSpesialisName", orderable: true, searchable: true},
									{ 
										data: "Alamat_1", orderable: true, searchable: true,
										render: function ( val ){
											try {
		...
		var _datatable = $( "#dt-lookup-suppliers" ).DT_Lookup_Suppliers();

		$('#dt-lookup-suppliers tbody').on( 'click', 'tr', function () {
			if ( $(this).hasClass('selected') ) {
				$(this).removeClass('selected');
			}else {
				$('#dt-lookup-suppliers tbody tr.selected').removeClass('selected');

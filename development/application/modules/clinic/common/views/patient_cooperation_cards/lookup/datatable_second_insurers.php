...
<div class="table-responsive">
    <table id="dt-lookup-patient_cooperation_cards" class="table table-sm table-bordered table-striped" width="100%">
        <thead>
            <tr>
                <th></th>
                <th>No Anggota</th>
...
(function( $ ){
		$.fn.extend({
				DT_Lookup_Patient_cooperation_cards: function(){
						var _this = this;
						...
						var _datatable = _this.DataTable( {
							...
							ajax: {
									...
									data: function( params ){
										...
										var PertanggunganKeduaCustomerKerjasamaID = _form.find( "input[id=\"PertanggunganKeduaCustomerKerjasamaID\"]" ).val();
										var NRM = _form.find( "input[id=\"NRM\"]" ).val();
										
										params.NRM = NRM;
										params.PertanggunganKeduaCustomerKerjasamaID = PertanggunganKeduaCustomerKerjasamaID;
									}
								},
							columns: [
									{ 
									...
									{ data: "Klp", orderable: true, searchable: true},
									{ 
		...
		var _datatable = $( "#dt-lookup-patient_cooperation_cards" ).DT_Lookup_Patient_cooperation_cards();

		$('#dt-lookup-patient_cooperation_cards tbody').on( 'click', 'tr', function () {
			if ( $(this).hasClass('selected') ) {
				$(this).removeClass('selected');
			}else {
				$('#dt-lookup-patient_cooperation_cards tbody tr.selected').removeClass('selected');

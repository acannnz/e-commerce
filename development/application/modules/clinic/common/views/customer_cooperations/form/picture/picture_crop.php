	...
	<div class="col-md-12">
        ...
        <div class="form-group">
            ...
            <div class="col-md-6">
                ...
                <a href="<?php echo base_url( "common/customer_cooperations/picture/{$profile_id}" ) ?>" data-toggle="ajax-modal" title="<?php echo lang("buttons:cancel") ?>" class="btn btn-block btn-cancel btn-danger"><i class="fa fa-times"></i> <?php echo lang("buttons:cancel") ?></a>
                <?php else: ?>
                <a href="<?php echo base_url( "common/customer_cooperations/picture/{$profile_id}" ) ?>" title="<?php echo lang("buttons:cancel") ?>" class="btn btn-block btn-cancel btn-danger"><i class="fa fa-times"></i> <?php echo lang("buttons:cancel") ?></a>
            	<?php endif ?>
            </div>
        </div>
		...
		$.fn.extend({
				form_picture_crop: function( picture_name, picture_type, elem_canvas, elem_preview ){
						...
						_picture_crop.init({			 
								...
								image: "<?php echo base_url( "resource/customer_cooperations/pictures" ) ?>/" + picture_name + "<?php echo (sprintf("?rand=%s", @time())) ?>",
								width: 240,
								height: 240,			 
								mask: true,			 
						...
						_this.on( "submit", function(e){
								...
								$.post( post_remote, post_data, function( data, status, xhr ){
										<?php if( isset($is_ajax_request) ): ?>
										try{
											ajax_modal.show( "<?php echo base_url( "common/customer_cooperations/picture/{$profile_id}" ) ?>" );
										} catch(e){}
										<?php else: ?>
										window.location = "<?php echo base_url( "common/customer_cooperations/picture/{$profile_id}" ) ?>";
										<?php endif ?>									
									} );

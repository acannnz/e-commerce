...
class Merchan extends Admin_Controller
{
	...
	protected $_model = 'merchan_m'; 
	
	public function __construct()
	{
		parent::__construct();
		$this->load->model( "merchan_m" );
		$this->page = "Merchan";
		$this->template->title( lang("merchan:page") . ' - ' . $this->config->item('company_name') );
	}
	...
	public function index()
	{
		...
		$this->template
			->set( "heading", lang("merchan:page") )
			->set_breadcrumb( lang("common:page"), base_url("common") )
			->set_breadcrumb( lang("merchan:breadcrumb") )
			->build('merchan/datatable', (isset($data) ? $data : NULL));
	}
	...
	public function lookup( $is_ajax_request=false )
	{
		...
		{
			$this->load->view( 'merchan/lookup/datatable' );
		} else
		{
			$data = array(
					'page' => $this->page,
			...
			$this->template
				...
				->build('merchan/lookup', (isset($data) ? $data : NULL));
		}
	}
	...
	public function datatable_collection( $state=false )
    {
		...
		$db_from = "{$this->merchan_m->table} a";
		$db_where = array();
		$db_like = array();
...
EOSQL;
	...
	public function autocomplete()
	{
		...
		$this->db
			->select( array("id", "code", "supplier_title") )
			;
			
		$this->db
			->from( "common_suppliers" )
			;
		...
		$this->db
			...
			->or_like(array(
					"code" => $words,
					"supplier_title" => $words,
					"supplier_description" => $words,
				))
			->group_end();

...
{
	public $table = 'mKategori';
	protected $index_key = 'Kategori_ID';
	...
	{	
		...
		{
			foreach($items as $item){ $populate[$item->Kategori_ID] = $item->Nama_Kategori; }
		}
	...
	public function to_list_html($first_label = '')
	{
		...
		if ($items = $this->get_all())
		{
			...
			{
				$option_html .= "<option value=\"{$item->Kategori_ID}\">{$item->Kode_Kategori} - {$item->Nama_Kategori}</option>";
			}
		}

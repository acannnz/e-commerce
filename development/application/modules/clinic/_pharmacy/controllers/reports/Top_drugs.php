...
class Top_drugs extends Admin_Controller
{
	...
	public function index()
	{
		...
		{
			redirect( "pharmacy/reports/top_drugs/dialog" );
		}
	}
	...
	public function dialog ( $is_ajax = FALSE)
	{
		...
		{
			$this->load->view(
				"reports/top_drugs/modal/dialog",
				array("form_child" => $this->load->view("reports/top_drugs/dialog", $data, true))
			);
		} else
		{
			$this->template
				...
				->build('reports/top_drugs/dialog', (isset($data) ? $data : NULL));
		}
	}
	...
	public function export_pdf()
	{
		if ($this->input->post())
		{
			...
			$collection = report_helper::get_top_drugs($post_data->date_start, $post_data->date_end);	
			$data = [
				"post_data" 	=> $post_data,	
				"collection" 	=> $collection,
			];
						
			$html_content =  $this->load->view( "reports/top_drugs/export/pdf", $data, TRUE ); 
	...
	public function export_excel()
	{
		...
		{
			$post_data = (object) $this->input->post('f');
			report_helper::export_excel_get_top_drugs($post_data->date_start, $post_data->date_end);

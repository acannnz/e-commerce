...
{ 
	...
	public function index()
	{
		...
		$this->template
			...
			->set_breadcrumb( lang("drug_payment:page"), base_url("pharmacy/drug-payment") )
			->build('pharmacy/drug_payment/datatable', (isset($data) ? $data : NULL));
	}
		
	public function pay( $NoFarmasi = 0 )
	{
		if ( $NoFarmasi == 0)	  
		{
			...
			redirect("pharmacy/drug-payment");
		}
	  
		$pharmacy = $this->db->where("NoBukti", $NoFarmasi)->get("BILLFarmasi")->row();
		$pharmacy->JenisKerjasama = $this->db->where("JenisKerjasamaID", $pharmacy->KerjasamaID)->get("SIMmJenisKerjasama")->row()->JenisKerjasama;
		$pharmacy_detail = $this->db->where("NoBukti", $NoFarmasi)->get("BILLFarmasiDetail")->result();

		$item = (object) array(
				'NoBukti' => drug_payment_helper::gen_evidence_number(),
				'NoBuktiFarmasi' => $NoFarmasi,
				'Tanggal' => date("Y-m-d H:i:s"),
				'Jam' => date("Y-m-d H:i:s"),
				'UserID' => $this->user_auth->User_ID,
				'NilaiPembayaran' => $pharmacy->Total,
		...
		if ( $pharmacy->ClosePayment == 1)	  
		{
			redirect("pharmacy/drug-payment/view/$NoFarmasi");
		}
		...
		if( $this->input->post() ) 
		{
			...
			{				
				...
				else
				{
					...
					redirect("pharmacy/drug-payment/view/$item->NoBuktiFarmasi");
				}				
		...
		$item->grandTotal = $pharmacy->Total; 
		...
		if( $this->input->is_ajax_request() )
		{
			$data = array(
					...
					"lookup_merchan" => base_url("pharmacy/drug-payment/lookup_merchan"),
		...
		} else
		{
			...
			$this->template
				...
				->set_breadcrumb( lang("drug_payment:breadcrumb"), base_url("pharmacy/drug_payment") )
	...
	public function view( $NoFarmasi = 0 )
	{
		...
		$item->GrandTotal = $item->NilaiTransaksi + $item->AddCharge;
		
		$pharmacy->JenisKerjasama = $this->db->where("JenisKerjasamaID", $pharmacy->KerjasamaID)->get("SIMmJenisKerjasama")->row()->JenisKerjasama;
		$pharmacy_detail = $this->db->where("NoBukti", $NoFarmasi)->get("BILLFarmasiDetail")->result();
		$merchan = $this->db->where("ID", $item->IDBank)->get("SIMmMerchan")->result();
		...
		if( $this->input->is_ajax_request() )
		{
			$data = array(
					...
					"pharmacy_detail" => $pharmacy_detail,
					"merchan" => $merchan,
					"cancel_payment_link" => base_url("pharmacy/drug-payment/cancel/$item->NoBuktiFarmasi"),
					"print_billing_link" => base_url("pharmacy/drug-payment/print_billing/$item->NoBuktiFarmasi"),
					"dp_billing_link" => base_url("pharmacy/drug-payment/dp_billing/$item->NoBuktiFarmasi"),
					"is_ajax_request" => TRUE,
					"is_modal" => TRUE,
					"is_edit" => FALSE,
				);
		...
		if( $this->input->post() ) 
		{
			...
			if( $item->NoBukti == $this->input->post( 'confirm' ) && $item->Audit == 0 )
			{
				...
				else
				{
					...
					redirect("pharmacy/drug-payment/pay/$item->NoBuktiFarmasi");
				}	
			}
	...
	public function dp_billing ( $NoFarmasi )
	{
		...
		try {
			...
			//$connector = new WindowsPrintConnector("smb://compaq:ea@KOHAKU/EPSON L110 Series"); 
			...
			$printer -> text(sprintf("%s : %s \n", lang("drug_payment:no_bill_label"), $item->NoBukti));
			if( !empty($item->NoReg)) 
			{
			...
			foreach($collection as $row) {
				...
				$left = sprintf("%sx %s", $row->Qty, number_format($sub_total[$row->NamaResepObat] / $row->Qty, 2, ".", ",") );

...
final class general_payment_report_helper
{
	...
	public static function export_transaction_recap_by_section_doctor( $date_start, $date_end, $section_id, $doctor_id )
	{
		$_ci = self::ci();
		...
		$file_name = sprintf('Laporan Rekap Pendapatan %s periode %s s/d %s', $doctor->Nama_Supplier, $date_start->format('d F Y'), $date_end->format('d F Y'));
		...
		foreach ( $collection['data'] as $key => $val ):
			...
			$_sheet->setCellValue("B{$tb_row}", @$val->ket);
				...
				->query("
					...
					(
						select SIMtrKasir.RJ, SIMtrRJ.Tanggal as TglTindakan,
								...
								sum(SIMtrRJTransaksi.Qty*
								SIMtrRJTransaksiDetail.Harga) as Nilai,
						...
						where SIMtrKasir.Batal=0
							...
							and SIMtrKasir.DokterID = '{$doctor_id}'
							and SIMtrKasir.SectionPerawatanID = '{$section_id}'
							and SIMtrRJTransaksiDetail.Harga <> 0
	...
	public static function export_transaction_recap_by_service_group( $date_start, $date_end, $section_id, $doctor_id )
	{
		$_ci = self::ci();
		...
		$date_end = DateTime::createFromFormat("Y-m-d", $date_end );
		$doctor = $_ci->supplier_model->get_by(['Kode_Supplier' => $doctor_id]);
		$file_name = sprintf('Laporan Rekap Pendapatan Dengan Grup Jasa %s periode %s s/d %s ', $doctor->Nama_Supplier, $date_start->format('d F Y'), $date_end->format('d F Y'));
		...
		$_sheet->setCellValue('A2', sprintf('%s Periode %s s/d %s ', $doctor->Nama_Supplier, $date_start->format('d F Y'), $date_end->format('d F Y')));
		$_sheet->getStyle("A2")->applyFromArray( self::_get_style( 'header' ) );

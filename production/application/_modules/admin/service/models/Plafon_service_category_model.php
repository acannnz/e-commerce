...
class Plafon_service_category_model extends CI_Model
{
	public $table = 'SIMmKategoriPlafon';
	public $index_key = 'KategoriPlafon';
	public $rules;
	...
	public function dropdown_html($where = NULL)
	{
		if($items = $this->get_all(NULL, 0, $where))
		{
			foreach($items as $item)
			{
				$option_html .= "<option value=\"{$item->{$this->index_key}}\">{$item->{$this->index_key}} - {$item->NamaAlat}</option>";
			}
		}	
		return $option_html;
	}

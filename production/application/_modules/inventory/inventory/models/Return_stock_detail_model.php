...
class Return_Stock_detail_model extends CI_Model
{
	public $table = 'BL_trReturDetail'; 
	...
	public function gen_request_number()
	{
		...
		$order_number = $this->db->query("SELECT MAX(No_Permintaan) AS No_Permintaan from BL_trPermintaan where year(Tgl_Permintaan) = '$y'")->row();
		//$order_number = $this->db->get();
		
		//print_r($order_number->No_Permintaan);exit;
		
		if (empty($order_number->No_Permintaan))
		{
			$kode = "$years"."/"."$month"."/"."PRQ"."/"."000001";
		} else {
			$order_number->No_Permintaan++;
			$kode = $order_number->No_Permintaan;
		}
		return (string) $kode;
	}
	...
	public function gen_request_id()
	{
		...
		$order_number = $this->db->query("SELECT MAX(Permintaan_ID) AS Permintaan_ID from BL_trPermintaan")->row();
		//$order_number = $this->db->get();
		
		//print_r($order_number->No_Permintaan);exit;
		
		$order_number->Permintaan_ID++;
		$kode = $order_number->Permintaan_ID;

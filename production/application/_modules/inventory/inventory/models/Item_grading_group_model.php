...
class Item_grading_group_model extends CI_Model
{
	public $table = 'SIMmKelompokGrading';
	protected $index_key = 'KelompokGrading';
	...
	{	
		...
		{
			foreach($items as $item){ $populate[$item->KelompokGrading] = $item->KelompokGrading; }
		}
	...
	public function to_list_html($first_label = '')
	{
		...
		if ($items = $this->get_all())
		{
			...
			{
				$option_html .= "<option value=\"{$item->KelompokGrading}\">{$item->KelompokGrading}</option>";
			}
		}
	...
	public function dropdown_data($where = NULL)
	{
		if($items = $this->get_all(NULL, 0, $where))
		{
			foreach($items as $item)
			{
				$option_data[$item->{$this->index_key}] = $item->{$this->index_key};
			}
		}	
		return $option_data;
	}

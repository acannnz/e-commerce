...
class Order_model extends CI_Model
{
	...
	/*
		@params
		(String) $key -> No_Penerimaan
	*/
	public function get_down_payment( $key, $to_array = FALSE )
	{
		$query = $this->db
			->select("COALESCE(Nilai_DP, 0) AS Nilai_DP, Order_ID")
			->from( $this->table )
			->where("Posting_Balance_DP", 0)
			->where("Order_ID IN ( SELECT Order_ID FROM BL_trPenerimaan WHERE No_Penerimaan = '{$key}')")
			->get();		

...
<div class="row">
	<div class="col-md-12">
    	<div class="form-group">
            <button type="button" id="select-product" class="btn btn-primary btn-block"><b><i class="fa fa-arrow-circle-down"></i> <?php echo lang('buttons:select') ?></b></button>
        </div>
    </div>
</div>
<?php echo form_close() ?>
...
(function( $ ){
		$.fn.extend({
				DT_Lookup_CommonServices: function(){
						...
						var _datatable = _this.DataTable( {
							...
							scrollY: "370px",
							ajax: {
									url: "<?php echo base_url("inventory/references/item/lookup_collection") ?>",
									type: "POST",
									data: function( params ){
										var _form = $("form[name=\"form_stock_opname\"]");
										if ( _form.size() > 0)
										{
											params.is_stock_opname = 1;
											params.location_id = _form.find("select[name=\"f[Lokasi_ID]\"]").val();
											params.type_group = _form.find("select[name=\"f[KelompokJenis]\"]").val();
										}
									}
								},
							columns: [
		...
		$('#select-all').on("change", function (e) {
			...
			if( $(this).is(':checked'))
			{
				$('#dt-lookup-common-services tbody tr').addClass('danger');
			} else {
				$('#dt-lookup-common-services tbody tr').removeClass('danger');
			}
		});

...
final class bank_cash_deposit_helper
{
	...
	public static function gen_evidence_number( )
	{
		...
		$query =  $CI->db->select("MAX(NoBukti) AS MAX")
							->where(array("Tipe" => "SKB", "Tanggal >=" => $date_start,"Tanggal <=" => $date_end))
		...
		if (!empty($query->MAX))
		{
			...
			$number++;
			$number = (string) (sprintf(self::_gen_format_evidence_number(), $date_y, $date_m, $date_d, 'SKB', $number));	
		} else {
			$number = (string) (sprintf(self::_gen_format_evidence_number(), $date_y, $date_m, $date_d, 'SKB', 1));		
		}
	...
	public static function get_bank_cash_deposit( $NoBukti )
	{
		$CI = self::ci();	

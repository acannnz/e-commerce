...
<body class="max-width max-height modal-open">
	...
	<script>
		$(document).on("ready", function(e) {
			...
			$(".box-top-patient").css("font-size", height * 6 / 100);
			...
			socket.onclose = function(e) {
				alert('Tidak dapat memanggil antrian, Websocket tidak aktif,  silahkan hubungi IT Support!');
				return false;
			};
			...
			var request = function name(data) {
				...
				var _response_mapping = data.split(',');
				var data_post = {};
					data_post['patient'] = {
			...
			var calling = function(response) {
					for (let i = 0, p = Promise.resolve(); i < call.length; i++) {	
						socket.send(["called_queue", response.data.NoReg]);
						p = p.then(_ => new Promise(resolve =>
							setTimeout(function() {
								$(".audioAntrean").html("");
								$('.audioAntrean').append(call[i])
								$("#playAnteran").click(); //trigegr play audio
			...
			socket.onmessage = function(e) {
				...
				{
					$.get('<?php echo base_url('queue/refresh_queue') ?>', function(response, status, xhr) {
						$.each(response.data, function(k, v) {
							$("#queue-number"+v.SectionID).html(v.queue)
						});
					});
				}else{

	...
	public static function create_registration($registration, $destinations, $patient, $vital)
	{
		...
		if (@$registration["PasienBaru"] == 1) {
			$_insert_patient = [
				...
				'PasienAsing' => @$patient['PasienAsing'], //tambahan (juli)
			];
		...
		$registration['PasienAsing'] = $patient['PasienAsing']; //tambahan (juli)
		...
		$vital['NoReg'] = $NoReg;
		$vital['NRM'] = $NRM;
		$vital['Height'] = (float) @$vital['Height'];
		$vital['Weight'] = (float) @$vital['Weight'];
		$vital['Temperature'] = (float) @$vital['Temperature'];

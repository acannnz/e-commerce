    ...
    function lookupbox_row_selected( response ){
        ...
        if( _response ){
            ...
            try {
				...
				$.get('<?php echo base_url("{$nameroutes}/get_service_component")?>', {ListHargaID: _response.ListHargaID}, function(data, textStatus, xhr){
					var addRow = {
						ListHargaID : _response.ListHargaID,
						Harga_Lama : _response.Harga_Lama,
						Harga_Baru : _response.Harga_Baru,
						Included : 0,
						TglPerubahanHarga : '',
						HonorDefault : 0,
						Honor : 0,
						Ditanggung : 0,
						Paket : 0,
						TglUpdate : '<?php echo date('Y-m-d')?>',
						AutoSystem : 0,
						JasaID : _response.JasaID,
						JasaName : _response.JasaName,
						NamaDokter : _response.NamaDokter,
						NamaKelas : _response.NamaKelas,
						Lokasi : _response.Lokasi,
						components: data.collection
					};
					$('#dt_contract_service').DataTable().row.add(addRow).draw();					
				});

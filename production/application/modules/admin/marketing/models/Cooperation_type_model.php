...
class Cooperation_type_model extends CI_Model
{
	public $table = 'SIMmJenisKerjasama';
	protected $index_key = 'JenisKerjasamaID';
	...
	public function dropdown_data($where = NULL)
	{
		if($items = $this->get_all(NULL, 0, $where))
		{
			...
			{
				$option_data[$item->{$this->index_key}] = $item->JenisKerjasama;
			}
		}	
		return $option_data;
	}
	...
	public function dropdown_html($where = NULL)
	{
		...
		if($items = $this->get_all(NULL, 0, $where))
		{
			foreach($items as $item)
			{
				$option_html .= "<option value=\"{$item->{$this->index_key}}\">{$item->{$this->index_key}} - {$item->JenisKerjasama}</option>";
			}
		} else {

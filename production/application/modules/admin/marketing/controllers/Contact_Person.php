...
class Contact_Person extends ADMIN_Controller
{
	protected $nameroutes = 'marketing/contact_person';
	...
	{
		$this->template
			->title(lang('heading:customer_contact_person'), lang('heading:customer'))
			->set_breadcrumb(lang('heading:customer') )
			->set_breadcrumb(lang('heading:customer__list'), site_url($this->nameroutes))
			->build("contact_person/datatable", $this->data);
	}
	...
	public function form( $row_index = NULL )
	{
		...
		{
			$this->load->view('contact_person/form', $this->data);
		}
	}
	...
	public function update($id = 0)
	{
		...
		if( $this->input->post() ) 
		{			
			$post_contact_person = $this->input->post("");
			...
			{								
				$this->db->trans_begin();
					...
					$this->contact_person_model->update( $post_contact_person, $id );	
		...
		$this->template
			->title(lang('heading:customer_contact_person'),lang('heading:customer'))
			->set_breadcrumb(lang('heading:customer__list'), site_url($this->nameroutes))
			->set_breadcrumb(lang('heading:customer__update'))
			->build("contact_person/form", $this->data);
	}
	...
	public function delete($id = 0)
	{
		...
		$this->load->view('contact_persons/modal/delete', $this->data);
	}
	...
	public function datatable_collection( $Status = 1 )
	{
		...
		$db_from = "{$this->contact_person_model->table} a";
		$db_where = array();
		$db_like = array();

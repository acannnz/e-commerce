...
class Service_group extends ADMIN_Controller
{
	protected $nameroutes = 'service/service_group';
	...
	public function index()
	{
		$this->template
			...
			->build("service_group/datatable", $this->data);
	}
	...
	public function create()
	{
		...
		if( $this->input->post() ) 
		{
			...
			$this->form_validation->set_rules($this->service_group_model->rules['insert']);
			$this->form_validation->set_data($post_group);
			if( $this->form_validation->run() )
			{							
				$this->db->trans_begin();
							
					$this->service_group_model->create( $post_group );							
		...
		$this->data['item'] = $item;
		$this->data['dropdown_treatment_group'] = $this->service_group_model->dropdown_static('KelompokPerawatan');
		...
		if(config_item('multi_bo') == 'TRUE'){
			...
			$this->data['lookup_oncall_account_2'] = base_url("{$this->nameroutes}/lookup_data/lookup_oncall_account_2");
			$this->data['lookup_ma_account_2'] = base_url("{$this->nameroutes}/lookup_data/lookup_ma_account_2");
		}
		...
		$this->template
			...
			->build("service_group/form", $this->data);
	}
	
	public function update($id = 0)
	{
		$this->data['item'] = $item = $this->service_group_model->get_one($id);
		...
		{			
			...
			$this->load->library('form_validation');
			$this->form_validation->set_rules($this->service_group_model->rules['update']);
			$this->form_validation->set_data($post_group);
			if( $this->form_validation->run() )
			{								
				$this->db->trans_begin();
				
					$this->service_group_model->update( $post_group, $id );	
		...
		$this->data['oncall'] = service_helper::get_service_group_account($item->AkunNoOnCall, 'BO_1');
		$this->data['ma'] = service_helper::get_service_group_account($item->AkunNoMA, 'BO_1');
		...
		if(config_item('multi_bo') == 'TRUE'){
			...
			$this->data['oncall_2'] = service_helper::get_service_group_account($item->AkunNoOnCall_2, 'BO_2');
			$this->data['ma_2'] = service_helper::get_service_group_account($item->AkunNoMA_2, 'BO_2');
	...
	public function delete($id = 0)
	{
		...
		{ 
			...
			$this->db->trans_begin();
				...
				$this->service_group_model->delete( $id );
		...
		$this->load->view('service_group/modal/delete', $this->data);
	}
	...
	{	
		...
		{
			$this->load->view("service_group/lookup/{$view}");
		}
	}
	...
	public function datatable_collection( $Status = 1 )
	{
		...
		$db_from = "{$this->service_group_model->table} a";
		$db_where = array();
		$db_like = array();

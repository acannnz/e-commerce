...
{
	protected $nameroutes = 'service/category';
	...
	public function create()
	{
		...
		$this->data['item'] = $item;
		$this->data['dropdown_service_group'] = $this->group_model->dropdown_data();
	...
	{	
		...
		{
			$this->load->view("category/lookup/{$view}");
		}
	}
	...
	public function dropdown_html( $parent_id=0 )
	{
		if( $this->input->is_ajax_request() )
		{
			...
			$collection = array();
			$collection = $this->category_model->dropdown_html( ['GroupJasa' => $parent_id] );
	...
	public function datatable_collection( $Status = 1 )
	{
		...
		if( isset($search['value']) && ! empty($search['value']) )
        {
			...
			$db_like[ $this->db->escape_str("a.{$this->category_model->index_key}") ] = $keywords;
		...
		$this->db
			->from( $db_from )
			->join("{$this->group_model->table} b", "a.GroupJasa = b.{$this->group_model->index_key}", "INNER")
			;
		if( !empty($db_where) ){ $this->db->where( $db_where ); }

...
class Drug_commission_patient_model extends CI_Model
{
	public $table = 'SIMdKomisiObatJenisPasien';
	public $index_key = 'DokterID';
	public $rules;
	...
	public function dropdown_data($where = NULL)
	{
		if($items = $this->get_all(NULL, 0, $where))
		{
			...
			{
				$option_data[$item->{$this->index_key}] = $item->SpesialisName;
			}
		}	
		return $option_data;
	}
	...
	public function to_list_html($first_label = '')
	{
		...
		if ($items = $this->get_all())
		{
			foreach($items as $item)
			{
				$option_html .= "<option value=\"{$item->{$this->index_key}}\">{$item->{$this->index_key}} - {$item->SpesialisName}</option>";
			}
		}

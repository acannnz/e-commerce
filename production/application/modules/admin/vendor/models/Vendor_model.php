...
class Vendor_model extends CI_Model
{
			...
			'insert' => [
				[
                	...
                	'label' => lang('label:code'),
               		'rules' => "required|is_unique[{$this->table}.Kode_Supplier]"
				],
				[
					'field' => 'Kode_Supplier_BPJS',
                	'label' => lang('label:code'). ' BPJS',
               		'rules' => "is_unique[{$this->table}.Kode_Supplier_BPJS]"
				],
				[
					'field' => 'Nama_Supplier',
	...
	public function to_list_html($first_label = '')
	{
		...
		if ($items = $this->get_all())
		{
			...
			{
				$option_html .= "<option value=\"{$item->Supplier_ID}\">{$item->Kode_Kategori} - {$item->Kategori_Name}</option>";
			}
		}

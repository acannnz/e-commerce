...
class Warehouse extends ADMIN_Controller
{
	...
	public function __construct()
	{
		...
		$this->data['nameroutes'] = $this->nameroutes = 'inventory/references/warehouse'; 
	...
	{
		$this->template
			->title(lang('heading:warehouses'))
			->set_breadcrumb(lang('heading:references'), site_url($this->nameroutes))
			->set_breadcrumb(lang('heading:warehouses'))
			->build("references/warehouse/index", $this->data);
	}
	
	public function create()
	{
		$this->data['form_action'] = $form_action = site_url("{$this->nameroutes}/create_post");
		$this->load->view('references/warehouse/modal/create', $this->data);
	}
	...
	public function update($id = 0)
	{
		...
		$this->load->view('references/warehouse/modal/update', $this->data);
	}
	...
	public function delete($id = 0)
	{
		...
		$this->load->view('references/warehouse/modal/delete', $this->data);
	}
	...
	public function create_post()
	{
		if( $this->input->post() ) 
		{
			...
			$this->form_validation->set_rules([
					['field' => 'Kode_Gudang', 'label' => lang('label:code'), 'rules' => 'required'],
					['field' => 'Nama_Gudang', 'label' => lang('label:name'), 'rules' => 'required']
				]);
	...
	public function collection()
	{
		...
		$this->datatables
			->select("Gudang_ID,Kode_Gudang,Nama_Gudang", false)
			->from($this->get_model()->table);
		
		$this->datatables->add_column("actions", $action, "Gudang_ID");		

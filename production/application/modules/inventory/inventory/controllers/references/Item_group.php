...
class Item_group extends ADMIN_Controller
{
	...
	public function __construct()
	{
		...
		$this->data['nameroutes'] = $this->nameroutes = 'inventory/references/item_group'; 
	...
	{
		$this->template
			->title(lang('heading:item_groups'),lang('heading:references'))
			->set_breadcrumb(lang('heading:references'), site_url($this->nameroutes))
			->set_breadcrumb(lang('heading:item_groups'))
			->build("references/item_group/index", $this->data);
	}
	
	public function create()
	{
		$this->data['form_action'] = $form_action = site_url("{$this->nameroutes}/create_post");
		$this->load->view('references/item_group/modal/create', $this->data);
	}
	...
	public function update($id = 0)
	{
		...
		$this->load->view('references/item_group/modal/update', $this->data);
	}
	...
	public function delete($id = 0)
	{
		...
		$this->load->view('references/item_group/modal/delete', $this->data);
	}
	...
	public function create_post()
	{
		if( $this->input->post() ) 
		{
			...
			$this->form_validation->set_rules([
					['field' => 'NamaGolongan', 'label' => lang('label:name'), 'rules' => 'required']
				]);
	...
	public function collection()
	{
		...
		$this->datatables
			->select("GolonganID,NamaGolongan")
			->from($this->get_model()->table);
		
		$this->datatables->add_column("actions", $action, "GolonganID");		

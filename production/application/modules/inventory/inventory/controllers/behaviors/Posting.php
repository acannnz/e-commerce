	...
	public function posting()
	{
		if ($this->input->post('confirm') == 1) {
			...
			if ($this->form_validation->run()) {
				...
				// Ambil satu data untuk pengecekan Order_ID
				$row = $this->posting_model->get_all($posting_data);
				if (!$row) {
					response_json(['status' => 'error', 'message' => lang('message:data_not_found')]);
				}

				// Cek Order_ID
				if (empty($row->Order_ID)) {
					$posting_data = $this->posting_model->get_selected_posting_data_not_order($posting_data);
				} else {
					$posting_data = $this->posting_model->get_selected_posting_data($posting_data);
				}

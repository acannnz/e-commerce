...
class Purchase_receipt extends Admin_Controller
{
	...
	public function dialog($is_ajax = FALSE)
	{
		...
		$option_category = $this->item_category_model->get_all();
	...
	private function export_excel()
	{
		if ($this->input->post()) {
			...
			$kategori = $this->input->post('f[Kategori_ID]');

			report_helper::export_purchase_receipt($date_start, $date_end, $location, $supplier, $opsi, $payment_type, $kelompok_jenis, $kategori);

...
class Rekap_Pembelian_Obat extends Admin_Controller
{
	protected $nameroutes = 'inventory/reports/rekap_pembelian_obat';
	...
	public function __construct()
	{
		...
		$this->page = 'Rekap Pembelian Obat';
	...
	public function dialog($is_ajax = FALSE)
	{
		...
		$populate_category = $this->item_category_model->dropdown_data();
		// print_r($populate_category);
		// exit;
		...
		$data = [
			...
			"populate_category" => $populate_category,
			...
			// "populate_category" => $this->item_category_model->to_list_data(),
		];

		if ($this->input->is_ajax_request() || $is_ajax) {
			$this->load->view(
				"reports/rekap_pembelian_obat/modal/dialog",
				array("form_child" => $this->load->view("reports/rekap_pembelian_obat/dialog", $data, true))
			);
		} else {
			$this->template
				->set("heading", $this->page)
				->set_breadcrumb('Laporan')
				->set_breadcrumb('Laporan Rekap Pembelian Obat', base_url("{$this->nameroutes}"))
				->build('reports/rekap_pembelian_obat/dialog', (isset($data) ? $data : NULL));
		}
	}
	...
	private function export_excel()
	{
		if ($this->input->post()) {
			...
			$populate_category = $this->input->post('f[Kategori_Id]');

			report_helper::export_rekap_pembelian_obat($date_start, $date_end, $location, $supplier, $opsi, $payment_type, $kelompok_jenis, $populate_category);
	...
	public function lookup_supplier($is_ajax_request = false)
	{
		if ($this->input->is_ajax_request() || $is_ajax_request !== false) {
			$this->load->view('reports/rekap_pembelian_obat/lookup/supplier');
		} else {

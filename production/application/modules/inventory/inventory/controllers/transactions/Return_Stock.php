...
class Return_Stock extends ADMIN_Controller
{
	...
	public function index()
	{
		...
		$this->template
			->title("Retur Stok Barang", lang('heading:transactions'))
			->set_breadcrumb(lang('heading:transactions'), site_url($this->nameroutes))
			->set_breadcrumb("Retur Stok Barang")
	...
	public function create_post()
	{
		if ($this->input->post()) {
			...
			if (!$this->form_validation->run()) {
				$post_data['User_ID'] = $this->user_auth->User_ID;
				$post_data['Tgl_Update'] = date("Y-m-d");
				$post_data['Supplier_ID'] = $extend['Supplier_ID'];
				$post_data['Lokasi_ID'] = $extend['Gudang_ID'];

				$this->db->trans_begin();
				if ($insert_id = $this->return_stock_model->create($post_data)) {
					foreach ($details as $i) {
						parse_str($i, $include);
						$include['Retur_ID'] = $insert_id;
					...
					foreach ($post_data_details as $rows) {
						...
						$exp_date    = $post_data['Tgl_Update'] != NULL ? $post_data['Tgl_Update'] : NULL;
				...
				if ($this->db->trans_status() === FALSE) {
					...
					$response = array(
						"Retur_ID" => $this->input->post("h[Retur_ID]"),
						"status" => 'success',
	...
	public function collection()
	{
		...
		$db_from = "{$this->return_stock_model->table} a";
		$db_where = array();
		$db_like = array();
		...
		if ($this->input->post("date_from")) {
			$db_where['a.Tgl_Retur >='] = $this->input->post("date_from");
		}

		if ($this->input->post("date_till")) {
			$db_where['a.Tgl_Retur <='] = $this->input->post("date_till");
		}
		...
		if (isset($search['value']) && !empty($search['value'])) {
			...
			$db_like[$this->db->escape_str("a.No_Retur")] = $keywords;
...
EOSQL;
	...
	public function get_subcategory_list($parent_id = 0)
	{
		...
		if (0 == $parent_id) {
			$parent_id = (int) $this->input->post('category_id', TRUE);
		}

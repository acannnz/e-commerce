...
final class report_helper
{
	...
	public static function export_purchase_receipt($date_start, $date_end, $location, $supplier, $opsi, $payment_type, $kelompok_jenis, $kategori)
	{
		$_ci = self::ci();
		...
		$collection = self::get_purchase_receipt($date_start, $date_end, $location, $supplier, $opsi, $payment_type, $kelompok_jenis, $kategori);
		...
		$spreadsheet->getProperties()->setCreator(config_item("company_name"))
			...
			->setTitle('Laporan Penerimaan Pembelian {$section->SectionName} ' . $payment_type_name[$payment_type] . $kelompok_jenis)
			->setSubject('Laporan Penerimaan Pembelian {$section->SectionName} ' . $payment_type_name[$payment_type] . $kelompok_jenis)
		...
		$_sheet->setCellValue('A2', 'Kelompok Jenis');
		$_sheet->setCellValue('B2', ': ' . $kelompok_jenis);
		$_sheet->getStyle("B2")->applyFromArray(['font'  => ['bold'	=> TRUE, 'size'  => 11]]);
	...
	public static function get_purchase_receipt($date_start, $date_end, $location, $supplier, $opsi, $payment_type, $kelompok_jenis, $kategori)
	{
		...
		if (!empty($kelompok_jenis)) {
			$db->where('e.KelompokJenis', $kelompok_jenis);
		}

		if (!empty($kategori != '')) {
			$db->where('f.Kategori_ID', $kategori);
		}
		...
		$db->select(" 
				...
				d.Nama_Supplier, b.Pajak AS PPN, b.Total_Nilai, b.No_DO, b.Potongan, b.Ongkos_Angkut, f.Nama_Kategori
			")
			...
			->join("SIMmKelompokJenisObat e", "c.KelompokJenis = e.KelompokJenis", "INNER")
			->join("mKategori f", "c.kategori_ID = f.Kategori_ID", "INNER")
			->where([
				'c.Kelompok' => 'OBAT',
	...
	public static function export_rekap_pembelian_obat($date_start, $date_end, $location, $supplier, $opsi, $payment_type, $kelompok_jenis, $populate_category)
	{
		$_ci = self::ci();
		...
		$collection = self::get_rekap_pembelian_obat($date_start, $date_end, $location, $supplier, $opsi, $payment_type, $kelompok_jenis, $populate_category);
		// print_r($collection);
		// exit;

		$date_start = DateTime::createFromFormat("Y-m-d", $date_start);
		$date_end = DateTime::createFromFormat("Y-m-d", $date_end);
		$file_name = sprintf("%s %s s/d %s", 'Laporan Rekap Pembelian Obat ' . $payment_type_name[$payment_type], $date_start->format(' d F Y'), $date_end->format('d F Y'));
		...
		$spreadsheet->getProperties()->setCreator(config_item("company_name"))
			->setLastModifiedBy(config_item("company_name"))
			->setTitle('Laporan Rekap Pembelian Obat {$section->SectionName} ' . $payment_type_name[$payment_type] . $kelompok_jenis)
			->setSubject('Laporan Rekap Pembelian Obat {$section->SectionName} ' . $payment_type_name[$payment_type] . $kelompok_jenis)
			->setDescription($file_name)
			->setKeywords($file_name);
		...
		if (empty($kelompok_jenis)) {
			$kelompok_jenis = 'Semua';
			$_sheet->setCellValue('A2', 'Kelompok Jenis');
			$_sheet->setCellValue('B2', ': ' . $kelompok_jenis);
			$_sheet->getStyle("B2")->applyFromArray(['font'  => ['bold'	=> TRUE, 'size'  => 11]]);
		}

		// if (empty($populate_category)) {
		// 	$populate_category = 'Semua';
		// 	$_sheet->setCellValue('A3', 'Kelompok Kategori');
		// 	$_sheet->setCellValue('B3', ': ' . $populate_category);
		// 	$_sheet->getStyle("B3")->applyFromArray(['font'  => ['bold'	=> TRUE, 'size'  => 11]]);
		// }
		...
		// $_sheet->setCellValue('A3', 'Kelompok Kategori');
		// $_sheet->setCellValue('B3', ': ' . $category_values);
		// $_sheet->getStyle("B3")->applyFromArray(['font'  => ['bold'	=> TRUE, 'size'  => 11]]);
		...
		foreach ($collection as $supplier => $facturs) :
			...
			foreach ($facturs as $factur => $items) :
				...
				$_sheet->setCellValue("C{$tb_row}", 'Keterangan');
				$_sheet->getStyle("C{$tb_row}")->applyFromArray(self::_get_style('thead'));
				$_sheet->setCellValue("D{$tb_row}", 'Sum Of Jumlah');
				$_sheet->getStyle("D{$tb_row}")->applyFromArray(self::_get_style('thead'));
				// $_sheet->setCellValue("E{$tb_row}", 'Satuan');
				// $_sheet->getStyle("E{$tb_row}")->applyFromArray(self::_get_style('thead'));
				$_sheet->setCellValue("E{$tb_row}", 'Harga@Satuan');
				$_sheet->getStyle("E{$tb_row}")->applyFromArray(self::_get_style('thead'));
				// $_sheet->setCellValue("G{$tb_row}", 'Diskon %');
				// $_sheet->getStyle("G{$tb_row}")->applyFromArray(self::_get_style('thead'));
				$_sheet->setCellValue("F{$tb_row}", 'Total');
				$_sheet->getStyle("F{$tb_row}")->applyFromArray(self::_get_style('thead'));
				$tb_row++;
				...
				foreach ($items  as  $item) :
					...
					if (empty($populate_category)) {
						$item->Nama_Kategori = 'Semua';
						$_sheet->setCellValue('A3', 'Kelompok Kategori');
						$_sheet->setCellValue('B3', ': ' . $item->Nama_Kategori);
						$_sheet->getStyle("B3")->applyFromArray(['font'  => ['bold'	=> TRUE, 'size'  => 11]]);
					}
					...
					// $_sheet->setCellValue("E{$tb_row}", $item->Kode_Satuan);
					// $_sheet->getStyle("E{$tb_row}")->applyFromArray(self::_get_style('tbody'));
					$_sheet->setCellValue("E{$tb_row}", $item->Harga_Beli);
					$_sheet->getStyle("E{$tb_row}")->applyFromArray(self::_get_style('tbody'));
					// $_sheet->setCellValue("G{$tb_row}", $item->Diskon_1);
					// $_sheet->getStyle("G{$tb_row}")->applyFromArray(self::_get_style('tbody'));
					$_sheet->setCellValue("F{$tb_row}", $item->Harga_Beli * $item->Qty);
					$_sheet->getStyle("F{$tb_row}")->applyFromArray(self::_get_style('tbody'));
					...
					// Menghitung subtotal
					$_sheet->setCellValue("F{$tb_row}", "=E{$tb_row} * D{$tb_row}");
					$_sheet->getStyle("F{$tb_row}")->applyFromArray(self::_get_style('tbody'));

					$sub_total += $item->Harga_Beli * $item->Qty;
					$tb_row++;
				...
				$_sheet->setCellValue("A{$tb_row}", 'SUB TOTAL');
				$_sheet->getStyle("A{$tb_row}:F{$tb_row}")->applyFromArray(self::_get_style('tfoot_value'));
				$_sheet->setCellValue("F{$tb_row}", $sub_total);
				$_sheet->getStyle("F{$tb_row}")->applyFromArray(self::_get_style('thead'));
				$_sheet->getStyle("F{$tb_row}")->applyFromArray(self::_get_style('currency'));
				$tb_row++;
		...
		// Menghitung dan mengatur grand total
		$_sheet->mergeCells("A{$tb_row}:E{$tb_row}");
		$_sheet->setCellValue("A{$tb_row}", 'GRAND TOTAL');
		$_sheet->getStyle("A{$tb_row}:F{$tb_row}")->applyFromArray(self::_get_style('tfoot_value'));
		$_sheet->setCellValue("F{$tb_row}", "=SUM(F7:F{$tb_row})/2");
		$_sheet->getStyle("F{$tb_row}")->applyFromArray(self::_get_style('thead'));
		$_sheet->getStyle("F{$tb_row}")->applyFromArray(self::_get_style('currency'));
	...
	public static function get_rekap_pembelian_obat($date_start, $date_end, $location, $supplier, $opsi, $payment_type, $kelompok_jenis, $populate_category)
	{
		...
		if (!empty($populate_category)) {
			$db->where('f.Kategori_ID', $populate_category);
		}

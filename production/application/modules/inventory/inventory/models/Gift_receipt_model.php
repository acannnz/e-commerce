...
class Gift_receipt_model extends CI_Model
{
	public $table = 'BL_trReceiveBonus'; 
	...
	public function gen_request_number()
	{
		...
		$order_number = $this->db->query("SELECT MAX(No_Bonus) AS No_Bonus from BL_trPenerimaan where year(Tgl_Bonus) = '$y'")->row();
		//$order_number = $this->db->get();
		
		//print_r($order_number->No_Permintaan);exit;
		
		if (empty($order_number->No_Bonus))
		{
			$kode = "$years"."/"."$month"."/"."RC"."/"."000001";
		} else {
			$order_number->No_Bonus++;
			$kode = $order_number->No_Bonus;
		}
		return (string) $kode;
	}

...
{
	public $table = 'SIMmGolongan';
	protected $index_key = 'GolonganID';
	...
	public function dropdown_data($where = NULL)
	{
		if($items = $this->get_all(NULL, 0, $where))
		{
			...
			{
				$option_data[$item->NamaGolongan] = $item->NamaGolongan;
			}
		}	
		return $option_data;
	}
	...
	{	
		...
		{
			foreach($items as $item){ $populate[$item->GolonganID] = $item->NamaGolongan; }
		}
	...
	public function to_list_html($first_label = '')
	{
		...
		if ($items = $this->get_all())
		{
			foreach($items as $item)
			{
				$option_html .= "<option value=\"{$item->GolonganID}\">{$item->NamaGolongan}</option>";
			}
		}

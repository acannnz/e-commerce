...
{		
	...
	public static function get_family_personal( $id, $where = NULL, $to_array = FALSE )
	{
		self::init();
		$_ci = self::ci(); 
	...
	public static function get_option_family_personal( $id )
	{
		self::init();
		$_ci = self::ci(); 
		
		$option_data = [];
		$collection = self::get_family_personal( $id );
	...
	public static function get_option_family_personal_by_relation( $family_id, $relation )
	{
		self::init();
		$_ci = self::ci(); 
		
		$option_data = [];
		$collection = self::get_family_personal( $family_id, ['b.Relation' => $relation] );
		foreach($collection as $row):
			$option_data[ $row->PersonalId ] = $row->PersonalName;
		endforeach;

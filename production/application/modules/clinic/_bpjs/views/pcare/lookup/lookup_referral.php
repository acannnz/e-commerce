...
function lookupbox_row_selected( response ){
	...
	if( _response ){
		try {
			...
			$('#tglEstRujuk').html( $('#lookupTglEstRujuk').val() );
			$('#kdppk').val(_response.kdppk);
			$('#providerRujukLanjut').html(_response.nmppk)
			...
			if(referralCondition == 'BPJSRujukanKhusus'){
				...
				$('#catatan').html($('#lookupCatatan').val());
				
				$('#poliRujukLanjut').html($('#lookupKdKhususSubSpesialis option:selected').html());
			} 
				...
				$('#poliRujukLanjut').html($('#lookupKdSubSpesialis1 option:selected').html());
			}
...
(function( $ ){
							...
							columns: [
									{ 
										...
										render: function ( val, type, row ){
											var json = JSON.stringify( {kdppk: row.kdppk, nmppk: row.nmppk} ).replace( /"/g, '\\"' );
									...
									{data: "nmppk", width: "200px",},
									{data: "kelas", className: 'text-center'},
									{data: "nmkc", width: "150px",},
									{data: "alamatPpk", width: "200px",},
									{data: "telpPpk", width: "100px", className: 'text-center'},
									{
										data: "distance", width: "80px", className: 'text-right',
										render: function(val){
									...
									{
										data: "jmlRujuk",  width: "100px", className: 'text-center',
										render: function(val, type, row){
											return val +' ('+ row.persentase +'%)'
										}
									},
									{data: "kapasitas",width: "100px", className: 'text-center'},
									{data: "jadwal", width: "150px", className: 'text-center'},
								]
						} );
		...
		var _form = {
			init: function(){
				...
				$('#lookupTglEstRujuk').datetimepicker({format: "DD-MM-YYYY"});

            ...
            <div class="panel-body">
				...
				<div class="form-group">
					<div class="col-lg-12 text-right">
						...
						<a href="<?php echo $cancel_link ?>" data-toggle="ajax-modal" class="btn btn-danger"><b><i class="fa fa-minus-square"></i> <?php echo lang( 'buttons:cancel' ) ?></b></a>
				        <a href="<?php echo $print_label ?>" target="_blank" class="btn btn-success"><b><i class="fa fa-print"></i> Cetak Label</b></a> 
				...
				edit: function( row, data, index ){
						...
						switch( this.index() ){							
							...
							case 4:
								...
								var _input = $( "<input type=\"text\" value=\"" + data.NoUrut + "\" style=\"width:100%\"  class=\"form-control\">" );
								this.empty().append( _input );
		...
		$( document ).ready(function(e) {
				...
				$("form[name=\"form_registrations\"]").on("submit", function(e){
					...
					table_data.each(function (v, i) {
						data_post.push({name: 'destinations['+ i +'][SectionID]', value: v.SectionID});
						data_post.push({name: 'destinations['+ i +'][DokterID]', value: v.DokterID});
						data_post.push({name: 'destinations['+ i +'][NoUrut]', value: v.NoUrut});
						data_post.push({name: 'destinations['+ i +'][WaktuID]', value: v.WaktuID});
						data_post.push({name: 'destinations['+ i +'][JenisKerjasamaID]', value: $("#JenisKerjasamaID").val()});
						data_post.push({name: 'destinations['+ i +'][UmurThn]', value: $("#UmurThn").val()});
						data_post.push({name: 'destinations['+ i +'][UmurBln]', value: $("#UmurBln").val()});
						data_post.push({name: 'destinations['+ i +'][UmurHr]', value: $("#UmurHr").val()});
												
						data_post.push({name: 'f[DokterRawatID]', value: v.DokterID});
					});
					
					if( $("#FamilyFolder").is(":checked") )
					{
						data_post.push({name: 'FamilyFolder', value: $("#FamilyFolder").val() || null});
					}
				...
				$("#JenisKerjasamaID").on("change", function(){
					if ( $(this).val() == 2 ){
						$(".second_insurer").val('');
						$(".second_insurer").prop("checked", false);
						$(".second_insurer").prop("disabled", true);
						$(".second_insurer").addClass("disabled");
					} else {
						$(".second_insurer").prop("disabled", false);
						$(".second_insurer").removeClass("disabled");
					}
				});

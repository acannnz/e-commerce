...
class Laboratory extends Public_Controller
{
	protected $_translation = "chart_file";	
	protected $_model = "chart_file_m";
	protected $_page = "files_chart_laboratory";
	
	private $_file_type = "laboratory";
	...
	public function update( $chart_number, $inline=FALSE )
	{
		...
		$data = array(
				"update_action" => base_url( "files/chart/laboratory/update/{$chart_number}" ),
				"file_upload_action" => base_url( "files/chart/laboratory/file_upload/{$chart_number}" ),
				"file_delete_action" => base_url( "files/chart/laboratory/file_delete/{$chart_number}" ),
		...
		if( $this->input->is_ajax_request() )
		{
			...
			echo ($this->load->view( "chart/laboratory/form/update", $data, TRUE ));
			exit(0);
		}
		...
		if( TRUE == $inline )
		{
			return ($this->load->view( "chart/laboratory/form/update", $data, TRUE ));
		}
		...
		$this->template
			...
			->build('chart/laboratory/form/update', $data);
	}
	...
	public function file_upload( $chart_number )
	{
		...
		if( $this->input->is_ajax_request() )
		{
			header("Content-Type: text/plain; charset=utf-8");
			echo ($this->load->view( 
					'chart/laboratory/modal/file_upload', 
					array('form_child' => $this->load->view('chart/laboratory/form/upload', $data, TRUE)),
					TRUE
				));
			exit(0);
		}
		...
		$this->template
			...
			->build('chart/laboratory/form/upload', $data);
	}
	...
	public function file_delete( $chart_number )
	{
		...
		{
			$this->get_model()->update(array("file_name" => NULL), array("chart_number" => $chart_number));				
		...
		header("Content-Type: text/plain; charset=utf-8");
		echo $this->load->view( 'chart/laboratory/modal/file_delete', $data, TRUE );
		exit(0);
	}
	...
	protected function _file_upload( & $file )
    {
		...
		if( $_FILES )
		{
			...
			{
				$config[ 'upload_path' ]	= './resource/patients/laboratory/';
				$config[ 'allowed_types' ]	= 'jpg';
				$config[ 'file_name' ]		= sprintf('PATIENT_%s_USG', strtoupper(str_replace(".","-",$file->mr_number)));
				...
				$this->get_model()->update(array("file_name" => $file->file_name), array("chart_number" => $file->chart_number));	
			}
		}

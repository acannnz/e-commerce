...
class Acyanotic_echocardiography extends Public_Controller
{
	protected $_translation = "chart_file";	
	protected $_model = "chart_file_m";
	protected $_page = "files_cardiology_acyanotic_echocardiography";
	
	private $_file_type = "cardiology_acyanotic_echocardiography";
	...
	public function update( $chart_number, $inline=FALSE )
	{
		...
		$data = array(
				"update_action" => base_url( "files/cardiology/acyanotic/echocardiography/update/{$chart_number}" ),
				"file_upload_action" => base_url( "files/cardiology/acyanotic/echocardiography/file_upload/{$chart_number}" ),
				"file_delete_action" => base_url( "files/cardiology/acyanotic/echocardiography/file_delete/{$chart_number}" ),
				"chart_number" => $chart_number,
				"item" => $item,
				"acyanotic_support" => "echocardiography",				
			);
		...
		if( $this->input->is_ajax_request() )
		{
			...
			echo ($this->load->view( "cardiology/acyanotic/echocardiography/form/update", $data, TRUE ));
			exit(0);
		}
		...
		if( TRUE == $inline )
		{
			return ($this->load->view( "cardiology/acyanotic/echocardiography/form/update", $data, TRUE ));
		}
		...
		$this->template
			...
			->build('cardiology/acyanotic/echocardiography/form/update', $data);
	}
	...
	public function file_upload( $chart_number )
	{
		...
		if( $this->input->is_ajax_request() )
		{
			header("Content-Type: text/plain; charset=utf-8");
			echo ($this->load->view( 
					'cardiology/acyanotic/echocardiography/modal/file_upload', 
					array('form_child' => $this->load->view('cardiology/acyanotic/echocardiography/form/upload', $data, TRUE)),
					TRUE
				));
			exit(0);
		}
		...
		$this->template
			...
			->build('cardiology/acyanotic/echocardiography/form/upload', $data);
	}
	...
	public function file_delete( $chart_number )
	{
		...
		header("Content-Type: text/plain; charset=utf-8");
		echo $this->load->view( 'cardiology/acyanotic/echocardiography/modal/file_delete', $data, TRUE );
		exit(0);
	}
	...
	protected function _file_upload( & $file )
    {
		...
		if( $_FILES )
		{
			if ( @file_exists( $_FILES[ 'userfile' ][ 'tmp_name' ] ) || @is_uploaded_file( $_FILES[ 'userfile' ][ 'tmp_name' ] ) )
			{
				...
				$config[ 'file_name' ]		= sprintf('PATIENT_%s_CARDILOGY_ACYANOTIC_ECHOCARDIOGRAPHY', strtoupper(str_replace(".","-",$file->mr_number)));

...
{
	protected $nameroutes = 'medical_record/most_diseases';
	...
	public function index()
	{
		...
		{
			echo "<script language=\"javascript\">window.location=\"".base_url("medical_record/most_diseases")."\";</script>";
			exit();
		} else
		{
			redirect( "medical_record/most_diseases/dialog" );
		}
	}
	...
	public function dialog ( $is_ajax = FALSE)
	{
		$data = array(
				"datepicker" => true,
				"form" => true,
				"url_export" => base_url("medical_record/most_diseases/export"),
			);
			
		if( $this->input->is_ajax_request() || $is_ajax )
		{
			$this->load->view(
				"medical_record/reports/most_diseases/modal/dialog",
				array("form_child" => $this->load->view("medical_record/reports/most_diseases/dialog", $data, true))
			);
		} else
		{
			$this->template
				->set( "heading", $this->page )
				->set_breadcrumb( $this->page, base_url("medical_record/most_diseases") )
				->set_breadcrumb( 'Laporan 10 Besar Penyakit' )
				->build('medical_record/reports/most_diseases/dialog', (isset($data) ? $data : NULL));
		}
	}
	...
	private function export_pdf()
	{
		...
		if ($this->input->post())
		{
			...
			$html_content =  $this->load->view( "medical_record/reports/most_diseases/export/pdf", $data, TRUE ); 

...
{
	protected $nameroutes = 'reports/daily_cash_report_fo';
	...
	public function dialog ( $is_ajax = FALSE)
	{
		...
		{
			$this->load->view(
				"reports/reports/daily_cash_report_fo/modal/dialog",
				array("form_child" => $this->load->view("reports/reports/daily_cash_report_fo/dialog", $data, true))
			);
		} else
		{
			$this->template
				...
				->build('reports/reports/daily_cash_report_fo/dialog', (isset($data) ? $data : NULL));
		}
	}
	...
	private function export_pdf()
	{
		...
		if ($this->input->post())
		{
			...
			$html_content =  $this->load->view( "reports/reports/daily_cash_report_fo/export/pdf", $data, TRUE ); 

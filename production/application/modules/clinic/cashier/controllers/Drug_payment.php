...
class Drug_payment extends Admin_Controller
{
	...
	public function view($NoFarmasi = 0)
	{
		...
		if ($this->input->is_ajax_request()) {
			$data = array(
				...
				"print_billing_link" => base_url("cashier/drug-payment/print_billing/"),
	...
	public function print_billing()
	{
		if ($this->input->post()) {
			...
			//TERBILANG
			$money_to_format = drug_payment_helper::money_to_text($grand_total + @$type_payment_used['Card Charge']);
			$money_to_format_english = drug_payment_helper::money_to_text_english($grand_total + @$type_payment_used['Card Charge']);
	...
	// 		// PDF Content
	// 		//$html_content = $this->load->view( "drug_payment/print/billing", $data, TRUE );    
	// 		$html_content = htmlentities($this->load->view("drug_payment/print/billing_table_only", $data, TRUE));

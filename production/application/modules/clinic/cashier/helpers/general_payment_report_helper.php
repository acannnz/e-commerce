...
final class general_payment_report_helper
{
	...
	public static function export_transaction_recap_by_section_doctor($date_start, $date_end, $section_id, $doctor_id = NULL)
	{
		...
		// TIPE BANK
		$_sheet->mergeCells("H{$tb_row}:I{$tb_row}");
		$_sheet->setCellValue("H{$tb_row}", "BANK");
		$_sheet->getStyle("H{$tb_row}")->applyFromArray(self::_get_style('header'));
		$tb_row++;

		$_sheet->setCellValue("H{$tb_row}", "Nama Bank");
		$_sheet->getStyle("H{$tb_row}")->applyFromArray(self::_get_style('thead'));
		$_sheet->setCellValue("I{$tb_row}", "Nilai");
		$_sheet->getStyle("I{$tb_row}")->applyFromArray(self::_get_style('thead'));
		$tb_row++;

		foreach ($collection['bank'] as $bank) :
			$_sheet->setCellValue("H{$tb_row}", @$bank->NamaBank);
			$_sheet->getStyle("H{$tb_row}")->applyFromArray(self::_get_style('tbody_merge'));
			$_sheet->setCellValue("I{$tb_row}", @$bank->Nilai);
			...
			$_total_bank += @$bank->Nilai;
			$tb_row++;
		endforeach;
		$_sheet->setCellValue("H{$tb_row}", "Total");
		$_sheet->getStyle("H{$tb_row}")->applyFromArray(self::_get_style('sum_name'));
		$_sheet->setCellValue("I{$tb_row}", @$_total_bank);
		$_sheet->getStyle("I{$tb_row}")->applyFromArray(self::_get_style('sum_value'));
		$_sheet->getStyle("I{$tb_row}")->applyFromArray(self::_get_style('currency'));
	...
	private static function _get_transaction_recap_by_section_doctor($date_start, $date_end, $section_id, $doctor_id)
	{
		...
		$collection = ['data' => [], 'component' => [], 'payment' => [], 'merchan' => [], 'bank' => []];
		...
		self::ci()->db->where_in('b.IDBayar', [7, 8]);
		...
		// Pasien dengan pembayaran Merchan (NAMA BANK)		
		if (!empty($doctor_id))
			self::ci()->db->where('a.DokterID', $doctor_id);
		self::ci()->db->where_in('b.IDBayar', [7, 8]);
		$collection['bank'] = self::ci()->db->select("d.NamaBank, SUM(b.NilaiBayar) AS Nilai")
			->from('SIMtrKasir a')
			->join('SIMtrKasirDetail b', 'a.NoBUkti = b.NoBukti', 'INNER')
			->join('mJenisBayar c', 'b.IDBayar = c.IDBayar', 'INNER')
			->join('SIMmMerchan d', 'a.IDBank = d.ID', 'INNER')
			->join('VW_Registrasi e', 'a.NoReg = e.NoReg', 'INNER')
			->where([
				'e.JamReg >=' => $date_start->format('Y-m-d H:i:s'),
				'e.JamReg <=' => $date_end->format('Y-m-d H:i:s'),

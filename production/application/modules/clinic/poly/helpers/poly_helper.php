...
final class poly_helper
{
	...
	public static function create_examination($rj, $diagnosis, $service, $service_component, $service_consumable, $vital, $soap, $nurse, $helper, $checkout, $consult, $patient = NULL, $odontogram)
	{
		...
		$_update_registration = [
			...
			'MeninggalTgl' => $checkout['MeninggalTgl'],
			'MeninggalJam' => $checkout['MeninggalJam'],
			'Keterangan' => @$patient['CatatanPatient'],
		];
		...
		if ($rj['TindakLanjutCekUpUlang']) :
			...
			$_reservation = [
				...
				'UntukTanggal' => substr($rj['TglCekUp'], 0, 10),
		...
		if (!empty($service)) {
			...
			foreach ($return_service_consumable as $val) {
				$_insert_fifo = [
					...
					'price' => $val["Harga"],
					'conversion' => 1,
					'evidence_number' => $rj['NoBukti'] . '-R-' . gen_unique_code(3),
					'trans_type_id' => 562,
					'in_out_state' => 1,
					'trans_date' => date('Y-m-d'),
					'exp_date' => date('Y-m-d'),
			...
			foreach ($service as $row) {
				...
				if (!empty($service_consumable[$row['JasaID']])) foreach ($service_consumable[$row['JasaID']] as $val) :
					...
					$_insert_fifo = [
						...
						'price' => $val["Harga"],
						'conversion' => 1,
						'evidence_number' => $rj['NoBukti'] . '-' . gen_unique_code(3),
						'trans_type_id' => 564,
						'in_out_state' => 0,
						'trans_date' => date('Y-m-d'),
						'exp_date' => date('Y-m-d'),
...
EOSQL;
	...
	public static function gen_evidence_number($SectionID = NULL, $NoReg = NULL, $Tanggal = NULL, $inpatient = FALSE)
	{
		...
		if ($section->KodeNoBukti == 'UMU') {
			$SectionID = array('SEC009', 'SEC082', 'SEC083');

			$query = $_ci->db->select("MAX(NoBukti) AS max_number,")
				->where_in('SectionID', $SectionID)
				->where([
...
EOSQL;
		...
		foreach ($query->result() as $row) :
			...
			$row->riwayat = sprintf(
				"Hpht = %s, Riwayat Menstruasi = %s, Riwayat Kehamilan = %s, Riwayat Persalinan Sebelumnya = %s, Riwayat KB = %s",
				$row->Hpht,
				$row->Rwt_Menstruasi,
				$row->Rwt_Kehamilan,
				$row->Rwt_Persalinan_Sebelumnya,
				$row->Rwt_KB
			);
			$collection[] = $row;
		endforeach;

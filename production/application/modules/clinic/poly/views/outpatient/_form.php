		...
		$(document).ready(function(e) {
			...
			$("form[name=\"form_poly\"]").on("submit", function(e) {
				...
				if ($('#vitalHeight').val() < 30 || $('#vitalHeight').val() > 250) {
					$.alert_warning("TINGGI BADAN HARUS DIATAS 30 DAN BIBAWAH 250 !!!")
					return false
				}
				if ($('#vitalWeight').val() < 2 || $('#vitalWeight').val() > 300) {
					$.alert_warning("BERAT BADAN HARUS DIATAS 2 DAN BIBAWAH 300 !!!")
					return false
				}
				if ($('#vitalTemperature').val() < 25 || $('#vitalTemperature').val() > 45) {
					$.alert_warning("SUHU TUBUH HARUS DIATAS 25 DAN BIBAWAH 45 !!!")
					return false
				}
				if ($('#vitalSystolic').val() < 40 || $('#vitalSystolic').val() > 250) {
					$.alert_warning("TEKANAN DARAH SYSTOLIC HARUS DIATAS 40 DAN BIBAWAH 250 !!!")
					return false
				}
				if ($('#vitalDiastolic').val() < 30 || $('#vitalDiastolic').val() > 180) {
					$.alert_warning("TEKANAN DARAH DIASTOLIC HARUS DIATAS 30 DAN BIBAWAH 180 !!!")
					return false
				}
				if ($('#vitalHeartRate').val() < 30 || $('#vitalHeartRate').val() > 160) {
					$.alert_warning("DETAK JANTUNG HARUS DIATAS 30 DAN BIBAWAH 160 !!!")
					return false
				}
				if ($('#vitalRespiratoryRate').val() < 5 || $('#vitalRespiratoryRate').val() > 70) {
					$.alert_warning("FREKUENSI PERNAPASAN HARUS DIATAS DAN BIBAWAH !!!")
					return false
				}
				if ($('#lingkarPerut').val() < 25 || $('#lingkarPerut').val() > 300) {
					$.alert_warning("LINGKAR PERUT HARUS DIATAS 25 DAN BIBAWAH 300 !!!")
					return false
				}

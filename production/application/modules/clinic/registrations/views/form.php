...
<input type="hidden" id="StatusPeriksa" value="<?php echo @$item->StatusPeriksa ?>">
<div class="row">
	<div class="col-md-12">
		<div class="panel">
					...
					<div id="poly-tab1" class="tab-pane tab-pane-padding active">
						...
						<div class="row">
							...
							<div class="col-md-6" hidden>
								<div class="form-group">
									<label class="control-label"><?php echo "Jam Reservasi" ?></label>
									<div class="input-group">
										<input type="text" id="JamReservasi" name="f[JamReservasi]" value="<?php echo @$item->JamReservasi ?>" placeholder="" class="form-control">
									</div>
								</div>
							</div>
						</div>
							...
							<div class="col-md-3">
								<div class="form-group">
									...
									<div class="input-group">
										...
										<span class="input-group-btn">
											<a href="<?php echo @$lookup_patients ?>" data-toggle="lookup-ajax-modal" class="btn btn-default lookup_patient"><i class="fa fa-search"></i></a>
						...
						<div class="row">
							...
							<div class="col-md-3">
								<div class="form-group">
									<label class="control-label"><?php echo lang('registrations:dob_label') ?></label>
						...
						<div class="row">
							...
							<div class="col-md-3">
								<div class="form-group">
									...
									<input id="KodeRegional" name="p[KodeRegional]" class="form-control patient regional hidden" value="<?= @$patient->KodeRegional ?>"></input>
								</div>
							</div>
						</div>
						<div class="page-subtitle margin-top-30">
						...
						<div class="row">
							...
							<div class="col-md-3">
								<div class="form-group">
									...
									<div class="form-group input-group">
										<input type="number" id="vitalTemperature" name="v[Temperature]" value="<?php echo $vital->Temperature ?>" placeholder="" class="form-control">
					...
					<div id="poly-tab4" class="tab-pane tab-pane-padding">
							...
							<div class="col-md-6">
								...
								<div class="form-group">
									...
									<div class="form-group">
										...
										<div id="room_selection" style="display: none;">
											<label class="col-md-2 control-label text-center">Kamar</label>
											<div class="col-md-5">
												<div class="input-group">
													<input type="hidden" id="NoBed" name="f[NoBed]" value="" placeholder="" class="form-control room">
													<input type="hidden" id="NoKamar" name="f[NoKamar]" value="" placeholder="" class="form-control room">
													<input type="text" id="Kamar" name="Kamar" value="" placeholder="" class="form-control room">
													<span class="input-group-btn">
														<a href="<?= base_url('registrations/lookup_room') ?>" data-toggle="lookup-ajax-modal" class="btn btn-default room"><i class="fa fa-search"></i></a>
								...
								<div class="form-group">
									<span class="btn-group btn-group-justified">
										<a href="<?php echo @$lookup_section ?>" <?php echo (@$is_edit && ( @$item->StatusPeriksa != 'Belum' || @$item->StatusBayar != "Belum" )) ? "disabled" : NULL ?> data-toggle="lookup-ajax-modal" class="btn btn-primary"><i class="fa fa-search"></i> Cari Section</a>
						...
						<div class="row">
							<div class="form-group">
								...
								<div class="col-md-4">
									<?php if (!@$is_view) : ?>
										<button type="submit" class="btn btn-primary btn-block " <?= (@$item->Batal == 1) ? "disabled" : null ?>><b><i class="fa fa-save"></i> <?php echo lang('buttons:submit') ?></b></button>
									<?php endif; ?>
								</div>
...
<script type="text/javascript">
	...
	var _form_actions = {
		...
		post: function(dataPost, fn) {
			...
			table_data.each(function(v, i) { 
				...
				dataPost.push({
					name: 'destinations[' + i + '][DokterID]',
					value: v.DokterID
				});
				dataPost.push({
					name: 'destinations[' + i + '][NoAntri]',
					value: v.NoAntri
				});
				dataPost.push({
					name: 'destinations[' + i + '][NoUrut]',
					value: v.NoUrut
				});
				dataPost.push({
				...
				dataPost.push({
					...
					value: $("#JenisKerjasamaID").val()
				});
				dataPost.push({
					name: 'destinations[' + i + '][UmurThn]',
					value: $("#UmurThn").val()
				});
				dataPost.push({
					name: 'destinations[' + i + '][UmurBln]',
					value: $("#UmurBln").val()
				});
				dataPost.push({
					name: 'destinations[' + i + '][UmurHr]',
					value: $("#UmurHr").val()
				});

				dataPost.push({
					name: 'f[DokterRawatID]',
					value: v.DokterID
				});
			});
	...
	var _datatable_actions = {
			...
			switch (this.index()) {
				...
				case 3:
					var _input = $("<input>", {
						...
						id: "Waktu",
						name: "f[Waktu]",
						value: "<?php echo substr(@$item->Waktu, 11, 5) ?>", // Mengambil hanya "HH:mm" dari format "HH:mm:ss"
						placeholder: ""
					});


					// var _input = $("<input type=\"text\" value=\"" + data.Waktu + "\" style=\"width:100%\"  class=\"form-control\">");
					...
					_input.on("blur", function(e) {
						e.preventDefault();
						try {
							var newValue = _input.val(); // Ambil nilai dari input datetimepicker
							data.Waktu = newValue; // Update nilai data.Waktu dengan nilai yang baru
							// _input.remove(); // Hapus input setelah selesai
		...
		check_type_service: function(index) {
			...
			$.alert_warning('Pelayanan Rawat Jalan hanya dapat memilih satu Section Tujuan saja');
				...
				_datatable = _this.DataTable({
					...
					columns: [{
							...
							render: function(val, type, row, meta) {
								return String("<a href=\"javascript:;\" <?php echo (@$is_edit && ( @$item->StatusPeriksa != 'Belum' || @$item->StatusBayar != "Belum" )) ? "disabled" : NULL ?> title=\"<?php echo lang("buttons:remov") ?>\" class=\"btn btn-danger btn-remove\"><i class=\"fa fa-times\"></i></a>")
							}
						},
						{
							data: "SectionName",
					...
					columnDefs: [{
						"targets": ["DokterID", "Waktu", "SectionID"],
						"visible": false,
						"searchable": false,
						// "defaultContent": "<?php echo @$item->Waktu ?>",
						// "targets": "_all"
					}],

...
class First_registration_patient extends Admin_Controller
{ 
	protected $_translation = 'reports';	
	protected $nameroutes = 'registrations/reports/first_registration_patient';
	...
	public function __construct()
	{
		...
		$this->page = 'Pasien Pertama Kali Registrasi';
	...
	public function dialog ( $ajax = FALSE)
	{
		...
		{
			$this->load->view(
				"reports/first_registration_patient/modal/dialog",
				array("form_child" => $this->load->view("reports/first_registration_patient/dialog", $data, true))
			);
		} else
		{
			$this->template
				...
				->build('reports/first_registration_patient/dialog', $data );
		}
	}
	...
	private function export_pdf()
	{
		if ($this->input->post())
		{
			...
			$collection = report_helper::get_first_registration_patient($post_data->date_start, $post_data->date_end );	
			...
			$html_content =  $this->load->view( "reports/first_registration_patient/export/pdf", $data, TRUE ); 
	...
	private function export_excel()
	{
		if ($this->input->post())
		{
			...
			$collection = reports_helper::get_first_registration_patient($post_data->date_start, $post_data->date_end );	

...
{ 
	...
	public function cost_breakdown( $NoBukti = NULL ){
		...
		$getdetailpasien = $this->db->select("NRM, NamaPasien, Alamat, JenisPasien, NoIdentitas")->where("NRM", $registration->NRM)->get( "mPasien" )->row();
		...
		export_helper::print_pdf($html_content, $file_name, date("Y-M-d"), $margin_bottom = 3.0, $header = NULL, $margin_top = 3.0, $orientation = 'L');
		exit(0);
	}
	...
	public function kwitansi( $NoBukti = NULL )
	{
		...
		if ( $this->input->is_ajax_request() || $NoBukti )		
		{
			...
			$data = array(
				"item" => $item,
				"spelled" => $spelled,
				"for_payment" => "Biaya Perawatan Rawat Jalan di Klinik Bali Sehat",
				"user" => $this->user_auth,
			);

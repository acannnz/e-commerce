...
class Outpatient extends Admin_Controller
{
	...
	public function note_patient($is_ajax_request = false, $NRM)
	{
	...
	public function create($NoReg, $SectionID)
	{
		...
		if ($this->input->post()) {
			...
			// if (!empty($vital)) {
			// 	$this->form_validation->set_rules($this->emr_vital_signs_model->rules['insert']);
			// 	$this->form_validation->set_data($this->input->post("vital"));
...
EOSQL;
	...
	public function get_shift()
	{
		if ($this->input->post()) {
			...
			$data = $this->db->where('IDShift', $IDShift)->get('SIMmShift')->row();

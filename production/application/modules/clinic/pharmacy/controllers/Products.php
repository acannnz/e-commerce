...
class Products extends Admin_Controller
{
	...
	public function __construct()
	{
		...
		//$test = $this->db->query("Select * from dbo.GetHargaObatNew_WithStok(3,'xx',0,1404,0,'". config_item('section_id') ."',0)")->row();		
		//print_r($test); exit;
	...
	public function product_package( )
	{
		$data = array(
			...
			$this->template
				...
				->set_breadcrumb( "pharmacy" )
				->set_breadcrumb( "Paket Obat", base_url("pharmacy/products") )
	...
	public function bhp_package( )
	{
		$data = array(
			...
			$this->template
				...
				->set_breadcrumb( "pharmacy" )
				->set_breadcrumb( "Paket BHP", base_url("pharmacy/products") )
	...
	{
		$item = array(
				'KodePaket' => pharmacy_helper::gen_product_package_number(), //Kode bisa: OBAT atau BHP
			);
		...
		$option_section = $this->pharmacy_m->get_options("SIMmSection");
		$lookup_supplier = base_url("pharmacy/emergencies/prescriptions/lookup_supplier");
		$lookup_product = base_url("pharmacy/products/lookup_product");
		...
		} else
		{
			...
			$this->template
				->set( "heading", "Buat Paket Obat" )
				->set_breadcrumb( "pharmacy" )
				->set_breadcrumb( "Paket Obat", base_url("pharmacy/products/product_package") )
	...
	public function product_package_edit( $KodePaket )
	{
		...
		$collection = $this->pharmacy_m->get_product_package_detail("SIMmDetailPaketObat", array("a.KodePaket" => $item_data['KodePaket']));
		$option_section = $this->pharmacy_m->get_options("SIMmSection");
		$lookup_supplier = base_url("pharmacy/emergencies/prescriptions/lookup_supplier");
		$lookup_product = base_url("pharmacy/products/lookup_product");
	...
	{
		$item = array(
				'Kode' => pharmacy_helper::gen_bhp_package_number(), //Kode bisa: OBAT atau BHP
			);
		...
		$option_section = $this->pharmacy_m->get_options("SIMmSection");
		$lookup_supplier = base_url("pharmacy/emergencies/prescriptions/lookup_supplier");
		$lookup_product = base_url("pharmacy/products/lookup_product/true/all_section");
		...
		} else
		{
			...
			$this->template
				->set( "heading", "Buat Paket BHP" )
				->set_breadcrumb( "pharmacy" )
				->set_breadcrumb( "Paket BHP", base_url("pharmacy/products/bhp_package") )
	...
	public function bhp_package_edit( $KodePaket )
	{
		...
		$collection = $this->pharmacy_m->get_bhp_package_detail("SIMmDetailPaketBHP", array("a.KodePaket" => $item_data['Kode']));
		$option_section = $this->pharmacy_m->get_options("SIMmSection");
		$lookup_supplier = base_url("pharmacy/emergencies/prescriptions/lookup_supplier");
		$lookup_product = base_url("pharmacy/products/lookup_product/true/all_section");
	...
	public function lookup_product_section( $is_ajax_request=false )
	{
		if( $this->input->is_ajax_request() || $is_ajax_request == true )
		{
			...
			$data = [
				'SectionID' => $section_pharmacy['section_id']
			];
			$this->load->view( 'products/lookup/datatable_section', $data );
		}
	}
	...
	public function datatable_product_section_collection()
    {
		...
		if ($this->input->get_post('SectionID'))
		{
			$farmasi = $this->pharmacy_m->get_row_data("SIMmSection", array("SectionID" => $this->input->get_post('SectionID')) );
...
EOSQL;
        ...
        $JenisKerjasamaID = $this->input->get_post("JenisKerjasamaID");
        $CustomerKerjasamaID = $this->input->get_post("CustomerKerjasamaID");
        $KTP = $this->input->get_post("KTP");
		$IsEmployee = (int) $this->input->get_post("IsEmployee");
		$section_pharmacy = $this->session->userdata('pharmacy');
        foreach($result as $row)
        {				
			# Params = JenisKerjasamaID, KelasID, KTP, Barang_ID, CustomerKerjasamaID, SectionID, JenisBarangID
			$HargaGrading = $this->db->query("Select * from GetHargaObatNew_WithStok($JenisKerjasamaID,'xx',$KTP,$row->Barang_ID,$CustomerKerjasamaID,'". $section_pharmacy['section_id'] ."',0)")->row();					
			$row->HNA = $HargaGrading->HPP_Baru;
			$row->HPP = $row->Harga_Beli;
			$row->Harga = $IsEmployee ? $row->Harga_Jual : $HargaGrading->Harga_Baru;
...
EOSQL;
        ...
        {
			// JenisKerjasamaID, KelasID, KTP, Barang_ID, CustomerKerjasamaID, SectionID, JenisBarangID
			$HargaGrading = $this->db->query("Select * from dbo.GetHargaObatNew_WithStok($JenisKerjasamaID,'xx',0,1404,$CustomerKerjasamaID,'". $section_pharmacy['section_id'] ."',0)")->row();		
			//print_r($test); exit;
			$row->HNA = $HargaGrading->HPP_Baru;
			$row->HPP = $row->Harga_Beli;
	...
	public function datatable_product_package_collection()
    {
		...
		{
			$farmasi = $this->pharmacy_m->get_row_data("SIMmSection", array("SectionID" => $this->input->post('SectionID')) );
			$db_where['a.Lokasi_ID'] = $farmasi->Lokasi_ID;
		}
...
EOSQL;
	...
	public function batch_collection()
    {		
		$db_from = "GD_trKartuBatch a";
		$db_where = array();
		$db_like = array();
		
		$db_where['a.Barang_ID'] = $this->input->get_post('Barang_ID');			
		$db_like[ $this->db->escape_str("a.NO_Batch") ] = $this->db->escape_str( $this->input->get_post('search') );

		$this->db->from( $db_from );
		$this->db->where( $db_where );
		$this->db->group_start()->or_like( $db_like )->group_end();		
		
      	$this->db->order_by('a.Kartu_ID', 'DESC');
		$this->db->limit(1);

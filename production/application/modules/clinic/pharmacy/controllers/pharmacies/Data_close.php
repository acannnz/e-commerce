...
class Data_close extends Admin_Controller
{
	...
	public function index()
	{
		...
		$this->load->view('pharmacies/datatable/close', (isset($data) ? $data : NULL));
	}
	...
	public function datatable_collection()
	{
		...
		if ($this->input->post("show_paid")) {
			$db_or_where['a.ClosePayment'] = $this->input->post("show_paid");
			unset($db_where['a.ClosePayment']);
		}
		if ($this->input->post("show_return")) {
			$db_or_where['a.Retur'] = $this->input->post("show_return");
			unset($db_where['a.Retur']);
		}
		if ($this->input->post("include_jasa")) {
			$db_or_where['a.IncludeJasa'] = $this->input->post("include_jasa");
			unset($db_where['a.IncludeJasa']);
		}

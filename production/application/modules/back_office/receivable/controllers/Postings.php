...
class Postings extends Admin_Controller
{
	...
	public function index()
	{
		$data = array(
				...
				"lookup_customers" => base_url("receivable/postings/lookup_customers"),
				"posting_url" => base_url("receivable/postings/posting"),
				"form" => TRUE,
				'datatables' => TRUE,
		...
		$this->template
			...
			->set_breadcrumb( lang("postings:page"), base_url("receivable/postings") )
	...
	public function cancel()
	{
		$data = array(
				...
				"beginning_balance_date" => receivable_helper::get_beginning_balance_date(),
				"lookup_customers" => base_url("receivable/postings/lookup_customers"),
				"posting_cancel_url" => base_url("receivable/postings/posting_cancel"),
				"form" => TRUE,
				'datatables' => TRUE,
				"navigation_minimized" => TRUE
			);
		...
		$this->template
			...
			->set_breadcrumb( lang("postings:page"), base_url("receivable/postings") )
			->set_breadcrumb( lang("postings:cancel_page"), base_url("receivable/postings/cancel") )
	...
	public function view_factur( $id=0 )
	{
		...
		if( $this->input->is_ajax_request() )
		{
			$data = array(
					...
					"customer" => $this->db->where("id", $this->item->customer_id)->get("common_customers")->row(),
	...
	public function view_invoice( $id=0 )
	{
		...
		$item = $this->db->where("id", $id)->get( "ar_invoices" )->row_array();
		$nota_detail = $this->db->where("invoice_number", $item['invoice_number'])->get( "ar_invoice_details" )->row();
		$invoice = $this->db->where("invoice_number", $nota_detail->evidence_number)->get( "ar_invoices" )->row();
		$invoice_detail = $this->db->where("invoice_number", $nota_detail->evidence_number)->get( "ar_invoice_details" )->row();
		...
		if( $this->input->is_ajax_request() )
		{
			...
			$this->load->view( 
					'postings/modal/create_edit', 
					array('form_child' => $this->load->view('postings/form_invoice', $data, true))
				);
		} else
		{
			$data = array(
			...
			$this->template
				...
				->set_breadcrumb( lang("receivables:page"), base_url("receivable/facturs") )
				->set_breadcrumb( lang("credit_debit_notes:page"), base_url("receivable/debit_credit_note") )
				->set_breadcrumb( lang("credit_debit_notes:view_heading") )
				->build('postings/form_invoice', $data);		
		}
	}	
	...
	public function datatable_collection( $posting_number = false )
    {
		...
		$db_factur = "AR_trFaktur a";
		$db_factur_where = array();
		$db_factur_where['a.Diakui_Piutang'] = 1;
		$db_factur_where['a.Cancel_Faktur'] = 0;
		$db_factur_where['a.Posted'] = 0;

		$db_invoice = "AR_trInvoice a";
		$db_invoice_where = array();
		$db_invoice_where['a.Cancel_Invoice'] = 0;
		$db_invoice_where['a.Posted'] = 0;

		/*$db_cashier = "GC_trGeneralCashier a";
		$db_cashier_where = array();
		$db_cashier_where['a.Type_Transaksi'] = 'ARC';
		...
		{
			$db_where['UNION_POSTED.Customer_ID'] = $this->input->post("Customer_ID");
		}
...
EOSQL;
		...
		$this->db
			...
			->join( "{$this->customer_m->table} e", "a.Customer_ID = e.Customer_ID", "INNER" )
...
EOSQL;
		...
		$this->db
			...
			->join( "{$this->customer_m->table} e", "a.Customer_ID = e.Customer_ID", "INNER" )
			->join( "mProyek f", "a.Kode_Proyek = f.Kode_Proyek_Real", "INNER" )
			->where_in( "a.JTransaksi_ID", array(202, 203, 205, 206, 207), FALSE );
		if( !empty($db_invoice_where) ){ $this->db->where( $db_invoice_where ); }
...
EOSQL;
		...
		$this->db
			...
			->group_by( array( 'a.Tgl_Transaksi', 'a.No_Bukti',  'd.Currency_Code', 'a.Keterangan',
						'e.Kode_Customer', 'e.Nama_Customer', 'e.Customer_ID', 'e.Akun_ID',
		...
		$union_posted = $this->db->from("
								...
								(
									{$_union_factur} UNION {$_union_invoice} 
								)
...
EOSQL;
	...
	public function lookup_accounts( $trId = NULL , $posting_id = NULL, $is_create = NULL ){
		...
		$post_action = ($is_create) ? base_url("receivable/postings/item_create") : base_url("receivable/postings/item_update");
	...
	public function lookup_customers( ){
		...
		if( $this->input->is_ajax_request() || $is_ajax_request !== false )
		{
			$this->load->view( 'postings/lookup/customers' );
		} 
	}
	...
	public function posting_cancel()
	{
		if ( $this->input->post() )
		{
			...
			$this->load->helper("Approval");
			$approver = $this->input->post('approver');
			if ( Approval_helper::approve( 'CANCEL POSTING AR', $approver['username'], $approver['password'] ) === FALSE )
			{
				$response["message"] = lang('auth_incorrect');
				$response["status"] = "error";
				$response["code"] = "500";				

...
class Factur extends Admin_Controller
{
	...
	public function index()
	{
		$data = array(
				...
				"lookup_customers" => base_url("receivable/factur/lookup_customers"),
				"form" => TRUE,
				'datatables' => TRUE,
		...
		$this->template
			->set( "heading", lang("facturs:page") )
					...
					->set_breadcrumb( lang("facturs:page"), base_url("receivable/factur") )
	...
	{
		$item_data = array(
				"No_Faktur" => receivable_helper::gen_factur_number( date("Y-m-d") ),
				"Currency_ID" => 1,
				"Customer_ID" => 0,
		...
		if( $this->input->post() ) 
		{
			...
			$header['No_Faktur'] = receivable_helper::gen_factur_number( $header['Tgl_Faktur'] );			
			...
			if( $this->input->is_ajax_request() )
			{
				$data = array(
						...
						"beginning_balance_date" => receivable_helper::get_beginning_balance_date(),
						"lookup_customers" => base_url("receivable/factur/lookup_customers"),
						"submit_url" => base_url("receivable/factur/create"),
						"create_url" => base_url("receivable/factur/create"),
					);
	...
	public function edit()
	{
		...
		if( $this->input->post() ) 
		{
			...
			if ( $this->get_model()->check_already_created_invoices( $No_Faktur ) == TRUE )
			{
				$response["message"] = lang('facturs:already_created_invoices');
				$response["status"] = "error";
				$response["code"] = "500";				
			...
			if( $this->input->is_ajax_request() )
			{
				$data = array(
						...
						"lookup_customers" => base_url("receivable/factur/lookup_customers"),
						"create_url" => base_url("receivable/factur/create"),
						"submit_url" => base_url("receivable/factur/edit")."?No_Faktur=". urlencode( $No_Faktur ),
						"cancel_url" => base_url("receivable/factur/cancel")."?No_Faktur=". urlencode( $No_Faktur ),
						"print_url" => base_url("receivable/facturs/export/factur")."?No_Faktur=". urlencode( $No_Faktur ),
					);
	...
	public function cancel()
	{
		...
		if( $this->input->post() ) 
		{
			...
			if ( Approval_helper::approve( 'CANCEL FAKTUR AR', $username, $password ) === FALSE )
			{
			...
			$data  = array(
				"item" => (object) $this->item->toArray(),
				"redirect_url" => base_url("receivable/factur/edit")."?No_Faktur=". urlencode( $No_Faktur ),
				"cancel_url" => base_url("receivable/factur/cancel")."?No_Faktur=". urlencode( $No_Faktur ),
			);
	...
	public function details( $factur, $is_edit = FALSE )
	{
		$data = array(
				...
				"form_action" => base_url("receivable/factur/items")."?No_Faktur=". urlencode($factur->No_Faktur),
				"populate_url" => base_url("receivable/factur/detail_collection")."?No_Faktur=". urlencode($factur->No_Faktur),
				"lookup_accounts" => base_url("receivable/factur/lookup_accounts"), 
				"form" => TRUE,
				"datatables" => TRUE,
			);
	...
	public function lookup_customers( ){
		...
		if( $this->input->is_ajax_request() )
		{
			$this->load->view( 'facturs/lookup/customers' );
		} 
	}
	...
	public function lookup_collection( $factur_number = false )
    {
		...
		// Default Filter		
		$db_where['a.Diakui_Piutang'] = 1;
		$db_where['a.Cancel_Faktur'] = 0;
		$db_or_where_group['a.No_Invoice'] = "-";
		$db_or_where_group['a.Cancel_Invoice'] = 1;
		
		if ( !empty( $this->input->post('Customer_ID') ))
		{
			$db_where['a.Customer_ID'] = $this->input->post('Customer_ID');
		}
...
EOSQL;
	...
	public function datatable_collection( $factur_number = false )
    {
		...
		{
			$db_where['a.JenisPiutang_ID'] = $filter['type_id'];		
		}
...
EOSQL;
		...
		$this->db
			->select( $db_select )
			->from( $db_from )
			->join("mCustomer b", "a.Customer_ID = b.Customer_ID", "INNER" )

...
<div class="modal-dialog modal-lg">
    <div class="modal-content">
        <div class="modal-header"> 
            ...
            <h4 class="modal-title"><?php echo lang('invoices:factur_lookup_title') ?></h4>
        </div>
        <div class="modal-body">
			...
			function lookupbox_row_selected( response ){
				...
				if( _response ){
					...
					check = $("#dt_invoice_details").DataTable().rows( function ( idx, data, node ) {
							return data.No_Faktur === _response.No_Faktur ?	true : false;
						} ).data();
					
					if ( check.any() )
					{	
						message = "<?php echo lang("invoices:factur_already_selected")?>";
					...
					try{
						...
						if ( rowIndex != "" )
						{							
							$("#dt_invoice_details").DataTable().row( rowIndex ).data( data ).draw();
						} else {
							$("#dt_invoice_details").DataTable().row.add( data ).draw();
						}
					...
					try{
						...
						$.each(response, function (index, value) {
							data = {
									...
									"Debit" : value.Nilai_Faktur,
									"Kredit" : 0,
									"JenisPiutang_ID" : value.JenisPiutang_ID
								};
						...
						$("#dt_invoice_details").DataTable().rows.add( data_table ).draw();
			...
			//]]></script>
            <?php  echo Modules::run( "receivable/factur/lookup", true ) ?>
        </div>
...
(function( $ ){
		...
		$( document ).ready(function(e) {
			...
			if ( $("#Customer_ID").val() == '' || $("#Customer_ID").val() == 0 )
			{
				...
				$.alert_error("<?php echo lang("invoices:customer_not_selected")?>");
				ajax_modal.show("<?php echo @$lookup_customers ?>")
			}

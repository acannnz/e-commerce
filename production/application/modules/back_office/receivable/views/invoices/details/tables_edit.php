...
<div class="row">
    <div class="col-md-12">
        <div class="table-responsive">
            <table id="dt_invoice_details" class="table table-bordered table-striped table-sm" width="100%">
                <thead>
                	<tr>
                    	<th></th>
...
(function( $ ){
		...
		var _datatable_actions = {		
				...
				calculate_balance: function(params, fn, scope){
						...
						var _text_balance = _form.find( "h3[id=\"invoice_value\"]" );
						...
						table_data.each(function (value, index) {
							tol_balance = tol_balance + Number(value.Debit);
						});
		...
		$.fn.extend({
				dt_invoice_details: function(){
						...
						_datatable = _this.DataTable( {
								...
								columnDefs  : [
										{
											"targets": ["Kredit","JenisPiutang_ID"],
											"visible": false,
											"searchable": false
										}
									],
						...
						$( "#dt_invoice_details_length select, #dt_invoice_details_filter input" )
		...
		$( document ).ready(function(e) {
            	$( "#dt_invoice_details" ).dt_invoice_details();
				
				$( "#JenisPiutang_ID" ).on("change", function(){
					_selected_data = $( this ).find( "option:selected" ).data();
					_datatable_actions.update_receivable_type( _selected_data );
				});
				...
				$("form[name=\"form_receivable\"]").on("submit", function(e){
					...
					var table_data = $( "#dt_invoice_details" ).DataTable().rows().data();
					table_data.each(function (value, index) {
						var detail = {

...
class Non_vouchers extends Admin_Controller
{
	protected $_translation = 'general_cashier';	
	protected $_model = 'cash_bank_expense/non_voucher_m';
	...
	public function index()
	{
		$data = array(
				...
				"lookup_customers" => base_url("general-cashier/cash-bank-expense/non-vouchers/lookup_customers"),
				"form" => TRUE,
				'datatables' => TRUE,
		...
		$this->template
			->set( "heading", lang("cash_bank_expense:non_voucher_list_heading") )
			->set_breadcrumb( lang("general_cashier:page") )
			->set_breadcrumb( lang("cash_bank_expense:page") )
			->set_breadcrumb( lang("cash_bank_expense:non_voucher_page"), base_url("general-cashier/cash-bank-expense/non-vouchers") )
			->build('cash_bank_expense/non_vouchers/datatable', (isset($data) ? $data : NULL));
	}
	...
	public function create(  )
	{
		...
		if( $this->input->post() ) 
		{
			...
			foreach($detail as $row)
			{
				if ( $row['Debit'] == 0 )
				{
					$response["message"] = sprintf(lang('cash_bank_expense:empty_account_value'), $row['No_Voucher'] );
					$response["status"] = "error";
					$response["code"] = "500";				
			...
			if( $this->input->is_ajax_request() )
			{
				$data = array(
						...
						"datatables" => TRUE,
						"web_stroge" => TRUE,
						"lookup_accounts" => base_url("general-cashier/cash-bank-expense/non-vouchers/lookup_accounts"),
						"lookup_accounts_credit" => base_url("general-cashier/cash-bank-expense/non-vouchers/lookup_accounts_credit"),
						"submit_url" => base_url("general-cashier/cash-bank-expense/non-vouchers/create"),
						"create_url" => base_url("general-cashier/cash-bank-expense/non-vouchers/create"),
					);
				
				$this->load->view( 
						'cash_bank_expense/non_vouchers/modal/create_edit', 
						array('form_child' => $this->load->view('cash_bank_expense/non_vouchers/form', $data, true))
					);
			} else
			{
				$data = array(
				...
				$this->template
					->set( "heading", lang("cash_bank_expense:non_voucher_create_heading") )
					->set_breadcrumb( lang("general_cashier:page") )
					->set_breadcrumb( lang("cash_bank_expense:page") )
					->set_breadcrumb( lang("cash_bank_expense:non_voucher_page"), base_url("general-cashier/cash-bank-expense/non-vouchers") )
					->set_breadcrumb( lang("cash_bank_expense:non_voucher_create_heading") )
					->build('cash_bank_expense/non_vouchers/form', $data);
			}
		}
	}
	...
	public function edit()
	{
		...
		if( $this->input->post() ) 
		{
			...
			foreach($detail as $row)
			{
				if ( $row['Debit'] == 0 )
				{
					$response["message"] = sprintf(lang('cash_bank_expense:empty_account_value'), $No_Bukti );
					$response["status"] = "error";
					$response["code"] = "500";				
			...
			if( $this->input->is_ajax_request() )
			{
				$data = array(
						...
						"lookup_accounts_credit" => base_url("general-cashier/cash-bank-expense/non-vouchers/lookup_accounts_credit"),
						"create_url" => base_url("general-cashier/cash-bank-expense/non-vouchers/create"),
						"submit_url" => base_url("general-cashier/cash-bank-expense/non-vouchers/edit")."?No_Bukti=". urlencode( $No_Bukti ),
						"cancel_url" => base_url("general-cashier/cash-bank-expense/non-vouchers/cancel")."?No_Bukti=". urlencode( $No_Bukti ),
					);
			...
			} else
			{
				...
				$this->template
					->set( "heading", lang("cash_bank_expense:non_voucher_edit_heading") )
					->set_breadcrumb( lang("general_cashier:page") )
					->set_breadcrumb( lang("cash_bank_expense:page") )
					->set_breadcrumb( lang("cash_bank_expense:non_voucher_page"), base_url("general-cashier/cash-bank-expense/non-vouchers") )
					->set_breadcrumb( lang("cash_bank_expense:non_voucher_edit_heading") )
					->build('cash_bank_expense/non_vouchers/form', $data);
			}
		}
	}
	...
	public function cancel()
	{
		...
		if( $this->input->post() ) 
		{
			...
			$data  = array(
				"item" => (object) $this->item->toArray(),
				"redirect_url" => base_url("general-cashier/cash-bank-expense/non-vouchers/edit")."?No_Bukti=". urlencode( $No_Bukti ),
				"cancel_url" => base_url("general-cashier/cash-bank-expense/non-vouchers/cancel")."?No_Bukti=". urlencode( $No_Bukti ),
			);
			
			$this->load->view( 'cash_bank_expense/non_vouchers/modal/cancel', $data );	
		}
	}
	...
	public function detail( $item, $is_edit = FALSE )
	{
		$data = array(
				...
				"lookup_vouchers" => base_url("general-cashier/cash-bank-expense/non-vouchers/lookup_vouchers"), 
				"form_voucher_detail_url" => base_url("general-cashier/cash-bank-expense/non-vouchers/lookup_form_voucher_detail"),
				"get_voucher_detail_url" => (!$is_edit) 
											? base_url("general-cashier/cash-bank-expense/non-vouchers/get_voucher_detail")
											: base_url("general-cashier/cash-bank-expense/non-vouchers/get_general_cashier_factur"),
			);
		
		$this->load->view( "cash_bank_expense/non_vouchers/details/accounts", $data );		
	...
	public function lookup( $is_ajax_request = false )
	{
		...
		{
			$this->load->view( 'cash_bank_expense/non_vouchers/lookup/datatable' );
		} 
	}	
	...
	public function lookup_accounts( $type = 'GC-Cash' ){
		...
		if( $this->input->is_ajax_request() || $is_ajax_request !== false )
		{
			...
			$this->load->view( 'cash_bank_expense/non_vouchers/lookup/accounts', $data );
		} 
	}
	...
	public function lookup_accounts_credit( $index = false ){
		...
		{			
			...
			$this->load->view( 'cash_bank_expense/non_vouchers/lookup/accounts_credit', $data );
		} 
	}

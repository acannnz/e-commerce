  ...
  <div class="modal-content">
    ...
    <h4 class="modal-title"><?php echo lang('edit_user')?></h4>
       ...
       $attributes = array('class' => 'bs-example form-horizontal');
          echo form_open(base_url().'users/accounts/auth',$attributes); ?>
          <?php
    ...
    <div class="modal-body">
      ...
      <div class="form-group">
        ...
        <div class="form-group">
        <label class="col-lg-4 control-label"><?php echo lang('role')?> <span class="text-danger">*</span></label>
        <div class="col-lg-8">
        <select name="role_id" class="form-control">
          <?php
          if (!empty($roles)) {
          foreach ($roles as $key => $role) { ?>
            <option value="<?php echo $role->r_id?>"<?php echo ($user->role_id == $role->r_id ? ' selected="selected"' : '')?>><?php echo ucfirst($role->role)?></option>
          <?php } } ?>          
        </select>
        </div>
        </div>

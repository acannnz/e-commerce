		...
		$.fn.extend({
				DT_Lookup_CommonAccount: function(){
						...
						var _datatable = _this.DataTable( {
							...
							//scrollY: "200px",
							ajax: {
									url: "<?php echo base_url("general_ledger/accounts/lookup_collection") ?>",
									type: "POST",
							...
							columns: [
									{ 
										...
										render: function ( val, type, row ){
												var data = {'id': row.id, 'account_number': row.account_number, 'account_name': row.account_name};
												var json = JSON.stringify( data ).replace( /"/g, '\\"' );
												return (row.is_child) ? "<a href='javascript:try{lookupbox_row_selected(\"" + json + "\")}catch(e){}' title=\"<?php echo lang( "buttons:apply" ) ?>\" class=\"btn btn-info btn-xs\"><i class=\"fa fa-check\"></i> <span><?php echo lang( "buttons:apply" ) ?></span></a>" : null;
											}
									},
									{ 
										data: "account_number",     

	...
	data: {
		...
		jQuery.extend(true, jQuery.fn.dataTable.defaults, {
			...
			language: {
				...
				<?php /*?>"processing": "<div class='table-loader'><span class='loading'></span></div>",<?php */ ?> "search": "<?php echo lang('datatable:search'); ?>",
	...
	<script type="text/javascript">
		...
		(function($) {
			$(document).ready(function(e) {
				...
				<?php if ("success" == $alert): ?>try {
					$("#audio-alert").get(0).play();
				} catch (ex) {}
			<?php endif ?>
			<?php if ("error" == $alert): ?>try {
...
<script>
	var AUTOSAVE_CONFIG = {
		user_id: '<?php echo $this->session->userdata('user_id') ?: 'guest'; ?>',
		csrf_token_name: '<?php echo $this->security->get_csrf_token_name(); ?>',
		autosave_expiry_minutes: 120
	};
</script>

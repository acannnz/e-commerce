...
class Template
{
	...
	public function build($view, $data = array(), $return = FALSE)
	{
		...
		if ($this->_layout)
		{
			...
			$this->_body =  $this->_load_view('layouts/'.$this->_layout, $this->_data, TRUE, self::_find_view_folder());
		}
	...
	/**
	 * Unset a view partial
	 *
	 * @access	public
	 * @param	string
	 * @return	void
	 */
	public function uns_partial( $name )
	{
	...
	{
		$this->_module = $module;
		//$this->_ci->router->module = $this->_module;
	...
	public function layout_exists($layout)
	{
		...
		return file_exists(self::_find_view_folder().'layouts/' . $layout . $this->_ext($layout));
	}
	 ...
	 * @return	array
	 */
	public function load_view($view, $data = array(), $parse_view = TRUE)
	{
		return $this->_find_view($view, (array) $data, $parse_view);
	}
	...
	private function _find_view($view, array $data, $parse_view = TRUE)
	{
		...
		if ( ! empty($this->_theme))
		{
			...
			*/
			...
			$module_name = isset($data["module"]) ? $data["module"] : $this->_module;
				
			$theme_views = array(
					$this->get_theme()."/".$this->get_views_path(true) . 'modules/' . $module_name . '/' . $view,
					// This allows build('pages/page') to still overload same as build('page')
					$this->get_theme()."/".$this->get_views_path(true) . 'modules/' . $view,
					$this->get_theme()."/".$this->get_views_path(true) . $view
				);
			...
			foreach ($this->_theme_locations as $location)
			{
				...
				{
					//print($location . $theme_view . $this->_ext($theme_view));
					//print "\n<br>";

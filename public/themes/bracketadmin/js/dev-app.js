...
var dev_settings_profile = {
   
   init: function(){ 

		/* Open/Hide Settings */
		$(".dev-settings-profile-button").on("click",function(){
			$(".dev-settings-profile").toggleClass("active");
		});
   }
};
...
var dev_header = {
    ...
    /* header actions */    
        ...
        $(".dph-buttons > li").on("click",function(e){
			...
			if($(this).find(".dev-logout").length > 0){
				try{
					var _this = $( e.target );				
					var remote = _this.data( 'remote' ) || _this.attr( 'href' );
					form_ajax_modal.show( remote );
				} catch(e){}
				return false;
            }            
        });
...
var dev_forms = {
    ...
    /* ./default form functions */
        ...
        if($("input[class*='mask_']").length > 0){        
            ...
            $("input.mask_nrm").mask('99.99.99');
            $("input.mask_rupiah").mask('Rp9.999');
        }
        /* ./masked inputs */
...
var ajax_modal = {
		...
		show: function( remote, fn, scope ){
				...
				var _modal = $("<div class=\"modal\" id=\"ajax-modal\" tabindex=\"-1\" aria-hidden=\"true\"><div class=\"modal-dialog\"><div class=\"modal-content\"><div class=\"modal-body\"><center><span class=\"label label-block label-danger\">Loading...</span></center></div></div></div></div>");
				$( "body" ).append( _modal );
				_modal.modal();
				_modal.load( remote );
...
var lookup_ajax_modal = {
		init: function(){
				$( document ).on('click', '[data-toggle="lookup-ajax-modal"]', function(e) {
						...
						lookup_ajax_modal.show( $remote )
					});
			},
		show: function( remote, fn, scope ){
				lookup_ajax_modal.hide()
				
				var _modal = $("<div class=\"modal\" id=\"lookup-ajax-modal\" tabindex=\"-1\" aria-hidden=\"true\"><div class=\"modal-dialog\"><div class=\"modal-content\"><div class=\"modal-body\"><center><span class=\"label label-block label-danger\">Loading...</span></center></div></div></div></div>");
				$( "body" ).append( _modal );
				_modal.modal();
				_modal.load( remote );
